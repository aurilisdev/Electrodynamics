...
public class OreGenReplace extends AbstractOreGenerator {
	...
	public int generateBranch(World world, Random rand, int chunkCornerX, int chunkCornerZ, int varX, int varY, int varZ) {
		int blocksPlaced = 0;
		List<Vector3d> pathed = new ArrayList();
	...
	public boolean isOreGeneratedInWorld(World world, IChunkProvider chunkGenerator) {
		return chunkGenerator instanceof ChunkProviderGenerate ? !this.ignoreSurface
				: chunkGenerator instanceof ChunkProviderHell ? !this.ignoreNether : chunkGenerator instanceof ChunkProviderEnd ? !this.ignoreEnd : false;
	}
}
