...
public class ElectricityDisplay implements IBaseUtilities {
	public static String getDisplay(double value, Unit unit, int decimalPlaces, boolean isShort) {
		...
		if (value == 0.0D) return value + " " + unitName;
		if (value <= Measurement.MILLI.value) return IBaseUtilities.roundPreciseStatic(Measurement.MICRO.process(value), decimalPlaces) + " " + Measurement.MICRO.getName(isShort) + unitName;
		if (value < 1.0D) return IBaseUtilities.roundPreciseStatic(Measurement.MILLI.process(value), decimalPlaces) + " " + Measurement.MILLI.getName(isShort) + unitName;
		else if (value > Measurement.MEGA.value) return IBaseUtilities.roundPreciseStatic(Measurement.MEGA.process(value), decimalPlaces) + " " + Measurement.MEGA.getName(isShort) + unitName;
		else if (value > Measurement.KILO.value) return IBaseUtilities.roundPreciseStatic(Measurement.KILO.process(value), decimalPlaces) + " " + Measurement.KILO.getName(isShort) + unitName;
		else if (value > Measurement.GIGA.value) return IBaseUtilities.roundPreciseStatic(Measurement.GIGA.process(value), decimalPlaces) + " " + Measurement.GIGA.getName(isShort) + unitName;
