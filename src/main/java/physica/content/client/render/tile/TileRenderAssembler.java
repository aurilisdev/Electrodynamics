...
public class TileRenderAssembler<T extends TileBaseRotateable> extends TileRenderObjModel implements IBaseUtilities {
	...
	@Override
	public void renderTileAt(TileBaseRotateable tile, double x, double y, double z, float deltaFrame) {
		super.renderTileAt(tile, x, y, z, deltaFrame);
		GL11.glPushMatrix();
		GL11.glTranslated(x + 0.5, y + 0.4, z + 0.5);
		GL11.glScalef(0.25f, 0.25f, 0.25f);
		TileAssembler assembler = (TileAssembler) tile;
		EntityItem entity = assembler.getEntityItem();
		if (entity != null)
		{
			entity.getEntityItem().stackSize = Math.max(1, Math.min(5, entity.getEntityItem().stackSize));
			entity.age = 1;
			entity.hoverStart = 0;
			renderItem.doRender(assembler.getEntityItem(), 0, 0, 0, 0, 5);
			float down = 4f;
			float forward = 0.125f;
			GL11.glScalef(1 / down, 0.333f, 1 / down);
			if (assembler.getOperatingTicks() > 0)
			{
				double centerX = assembler.xCoord + tile.getFacing().offsetX * down * forward + 0.5 * down, centerY = assembler.yCoord + 0.4 * down,
						centerZ = assembler.xCoord + tile.getFacing().offsetX * down * forward + 0.5 * down;

				double targetX = assembler.xCoord + tile.getFacing().offsetX * down * forward + 0.5 * down + tile.getWorldObj().rand.nextFloat() / down - 1 / down / 1.333,
						targetY = assembler.yCoord + 0.9 * down * 2,
						targetZ = assembler.xCoord + tile.getFacing().offsetX * down * forward + 0.5 * down + tile.getWorldObj().rand.nextFloat() / down - 1 / down / 1.333;
				...
				for (int i = 0; i < 5; i++)
				{
					float u1 = -(assembler.getOperatingTicks() * deltaFrame * tile.getWorldObj().rand.nextFloat() * (i + 1) * 0.005F);
					...
					for (int j = 0; j <= total; ++j)
					{
						...
						tessellator.setColorOpaque(200 - Math.abs(120 / 2 - assembler.getOperatingTicks() % 10 * 12), 0, 0);
