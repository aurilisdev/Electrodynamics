...
public class RecipeManagerPluginCanister implements IRecipeManagerPlugin {
	...
	public <V> List<RecipeType<?>> getRecipeTypes(IFocus<V> focus) {
		...
		if (focus.getRole() == RecipeIngredientRole.RENDER_ONLY) {
			return Collections.emptyList();
		}
		...
		if (jeiDefaultManager == null) {
			handleRelection(runtime);
		}
	...
	private void handleRelection(JeiRuntime runtime) {
		...
		if (!(generic instanceof RecipeManager)) {
			return;
		}
			...
			try {
				...
				try {
					...
					for (IRecipeManagerPlugin plugin : plugins) {
						if (plugin instanceof InternalRecipeManagerPlugin) {
							jeiDefaultManager = (InternalRecipeManagerPlugin) plugin;
							break;
						}
					}

				} catch (Exception e) {
					Electrodynamics.LOGGER.info("Failed to get List<IRecipeManagerPlugin> field from PluginManager");
				}

			} catch (Exception e) {
				Electrodynamics.LOGGER.info("Failed to get PluginManager field from RecipeManagerInternal");
			}
