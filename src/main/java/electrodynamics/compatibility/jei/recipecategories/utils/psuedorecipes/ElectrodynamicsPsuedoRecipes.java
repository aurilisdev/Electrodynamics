...
public class ElectrodynamicsPsuedoRecipes {
	...
	for (Gas gas : VoltaicRegistries.gasRegistry().getValues()) {
	    ...
	    inputHandler.fill(new GasStack(gas, 1000, gas.getCondensationTemp() + 1, Gas.PRESSURE_AT_SEA_LEVEL),
		    GasAction.EXECUTE);

	    ItemStack outputBucket = new ItemStack(gas.getCondensedFluid().getBucket());

	    IFluidHandlerItem outputHandler = outputBucket.getCapability(ForgeCapabilities.FLUID_HANDLER_ITEM)
	    ...
	    CONDENSATION_RECIPES
		    .add(new PsuedoGas2FluidRecipe(Arrays.asList(new GasStack(gas, 1000, gas.getCondensationTemp(), 1)),
			    new FluidStack(gas.getCondensedFluid(), 1000), inputCylinder, outputBucket));
	...
	VoltaicGases.MAPPED_GASSES.forEach((fluid, gas) -> {
	    ...
	    IFluidHandlerItem inputHandler = inputBucket.getCapability(ForgeCapabilities.FLUID_HANDLER_ITEM)
		    .orElse(CapabilityUtils.EMPTY_FLUID_ITEM);
	    ...
	    outputHandler.fill(new GasStack(gas, 1000, gas.getCondensationTemp() + 1, Gas.PRESSURE_AT_SEA_LEVEL),
		    GasAction.EXECUTE);

	    outputCylinder = outputHandler.getContainer();

	    EVAPORATION_RECIPES.add(new PsuedoFluid2GasRecipe(Arrays.asList(new FluidStack(fluid, 1000)),
		    new GasStack(gas, 1000, gas.getCondensationTemp(), 1), inputBucket, outputCylinder));
