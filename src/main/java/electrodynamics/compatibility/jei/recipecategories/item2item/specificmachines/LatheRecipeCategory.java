...
public class LatheRecipeCategory extends Item2ItemRecipeCategory<LatheRecipe> {
    ...
    public static ItemStack INPUT_MACHINE = new ItemStack(
	    ElectrodynamicsBlocks.BLOCKS_MACHINE.getValue(SubtypeMachine.lathe));

    public static final RecipeType<LatheRecipe> RECIPE_TYPE = RecipeType.create(Electrodynamics.ID,
	    LatheRecipe.RECIPE_GROUP, LatheRecipe.class);

    public LatheRecipeCategory(IGuiHelper guiHelper) {
	super(guiHelper, ElectroTextUtils.jeiTranslated(LatheRecipe.RECIPE_GROUP), INPUT_MACHINE, BACK_WRAP,
