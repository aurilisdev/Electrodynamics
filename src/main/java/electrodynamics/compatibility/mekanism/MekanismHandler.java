...
public class MekanismHandler {
    ...
    public static final SinglePropertyType<ChemicalStack, RegistryFriendlyByteBuf> CHEMICAL_STACK = new SinglePropertyType<>(
		...
		fluidTag = ChemicalStack.OPTIONAL_CODEC.encode(writer.prop().getValue(), NbtOps.INSTANCE, fluidTag)
			.getOrThrow();
		writer.tag().put(writer.prop().getName(), fluidTag);
	    },
	    //
	    reader -> ChemicalStack.OPTIONAL_CODEC
		    .decode(NbtOps.INSTANCE, reader.tag().getCompound(reader.prop().getName())).getOrThrow().getFirst()
    //
    );
	...
	if (tile.conversionIsFlipped.getValue()) {
	    ...
	    GasStack gas = tank.getGas();
	    Chemical chemical = GasMapReloadListener.INSTANCE.gasToChemicalMap.get(gas.getGas());
	    int accepted = (int) Math.min(rate,
		    Math.max(gas.getAmount(), TileRotaryUnifier.MAX_CHEM_AMOUNT - prop.getValue().getAmount()));
