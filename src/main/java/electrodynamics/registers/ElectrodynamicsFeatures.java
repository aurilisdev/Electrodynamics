	...
	static {
		...
		for (SubtypeOre ore : SubtypeOre.values()) {
			if (OreConfig.oresToSpawn.contains(ore.name())) {
				Supplier<List<OreConfiguration.TargetBlockState>> targetSupplier = Suppliers.memoize(() -> List.of(OreConfiguration.target(OreFeatures.STONE_ORE_REPLACEABLES, UnifiedElectrodynamicsRegister.getSafeBlock(ore).defaultBlockState()), OreConfiguration.target(OreFeatures.DEEPSLATE_ORE_REPLACEABLES, UnifiedElectrodynamicsRegister.getSafeBlock(SubtypeOreDeepslate.values()[ore.ordinal()]).defaultBlockState())));
