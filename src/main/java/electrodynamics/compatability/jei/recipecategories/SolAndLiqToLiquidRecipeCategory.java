...
public abstract class SolAndLiqToLiquidRecipeCategory implements IRecipeCategory<PsuedoSolAndLiqToLiquidRecipe> {
    ...
    public SolAndLiqToLiquidRecipeCategory(IGuiHelper guiHelper, String modID, String recipeGroup, String guiTexture, ItemStack inputMachine,
	...
	MajorProcessingArrowCoords = inputCoordinates.get(1);
	MinorProcessingArrowCoords = inputCoordinates.get(2);

	InputItemOffset = inputCoordinates.get(3);
	InputFluidBucketOffset = inputCoordinates.get(4);
	InputFluidTank = inputCoordinates.get(5);
	OutputFluidTank = inputCoordinates.get(6);

	MajorProcessingArrowOffset = inputCoordinates.get(7);
	MinorProcessingArrowOffset = inputCoordinates.get(8);

	RightTankFluidBar = inputCoordinates.get(9);

	majorArrowStartDirection = majorArrowDirection;
	minorArrowStartDirection = minorArrowDirection;
	...
	cachedArrows = CacheBuilder.newBuilder().maximumSize(25).build(new CacheLoader<Integer, ArrayList<IDrawableAnimated>>() {
		...
		IDrawableAnimated largeArrow = guiHelper.drawableBuilder(GUI_TEXTURE, MajorProcessingArrowCoords[0], MajorProcessingArrowCoords[1],
			MajorProcessingArrowCoords[2], MajorProcessingArrowCoords[3]).buildAnimated(cookTime, majorArrowStartDirection, false);

		IDrawableAnimated smallArrow = guiHelper.drawableBuilder(GUI_TEXTURE, MinorProcessingArrowCoords[0], MinorProcessingArrowCoords[1],
			MinorProcessingArrowCoords[2], MinorProcessingArrowCoords[3]).buildAnimated(cookTime, minorArrowStartDirection, false);
	...
	cachedFluidBars = CacheBuilder.newBuilder().maximumSize(25).build(new CacheLoader<Integer, IDrawableStatic>() {

	    @Override
	    public IDrawableStatic load(Integer cookTime) {

		outputFluidBar = guiHelper
			.drawableBuilder(GUI_TEXTURE, RightTankFluidBar[0], RightTankFluidBar[1], RightTankFluidBar[2], RightTankFluidBar[3]).build();
	...
	guiFluidStacks.init(INPUT_FLUID_STACK_SLOT, true, InputFluidTank[0], leftStartY, InputFluidTank[2], leftHeightOffset, fluidInputAmount, true,
		null);
	guiFluidStacks.init(OUTPUT_FLUID_SLOT, false, OutputFluidTank[0], rightStartY, OutputFluidTank[2], rightHeightOffset, fluidOutputAmount, true,
		null);
	...
	getFluidBar(recipe).draw(matrixStack, OutputFluidTank[0], rightBarStartHeight, OutputFluidTank[3] - rightHeightOffset, 0, 0, 0);
