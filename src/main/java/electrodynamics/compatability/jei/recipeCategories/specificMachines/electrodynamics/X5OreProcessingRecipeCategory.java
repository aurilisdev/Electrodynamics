...
public class X5OreProcessingRecipeCategory implements IRecipeCategory<Psuedo5XRecipe>{
	...
	//private static Logger logger = LogManager.getLogger(ElectrodynamicsPatches.MOD_ID);
		...
		this.cachedArrows = CacheBuilder.newBuilder()
				...
				.build(new CacheLoader<Integer, IDrawableAnimated>() 
				{
					...
					public IDrawableAnimated load(Integer cookTime) {
						return guiHelper.drawableBuilder(GUI_TEXTURE, 0, 58, 88, 16)
	...
	protected void drawSmeltTime(Psuedo5XRecipe recipe, MatrixStack matrixStack, int y) 
	{
		...
		TranslationTextComponent mineralWasherString = 
				new TranslationTextComponent("gui.jei.category."+RECIPE_GROUP+".info.mineral_washer", smeltTimeSeconds);
		TranslationTextComponent chemicalCrystalizerString = 
				new TranslationTextComponent("gui.jei.category."+RECIPE_GROUP+".info.chemical_crystalizer", smeltTimeSeconds);
