...
public class ElectrodynamicsItemTagsProvider extends ItemTagsProvider {
	...
	public ElectrodynamicsItemTagsProvider(DataGenerator generator, BlockTagsProvider provider, ExistingFileHelper existingFileHelper) {
		super(generator, provider, Electrodynamics.ID, existingFileHelper);
	}
	...
	protected void addTags() {
		...
		Builder<Item> gears = tag(VoltaicTags.Items.GEARS);

		for (SubtypeGear gear : SubtypeGear.values()) {
			gears.addTag(gear.tag);
		}

		Builder<Item> ingots = tag(VoltaicTags.Items.INGOTS);

		for (SubtypeIngot ingot : SubtypeIngot.values()) {
			ingots.addTag(ingot.tag);
		}

		Builder<Item> ores = tag(VoltaicTags.Items.ORES);

		for (SubtypeOre ore : SubtypeOre.values()) {
			ores.addTag(ore.itemTag);
		}
		
		tag(VoltaicTags.Items.CONCRETES).add(ElectrodynamicsItems.ITEMS_CONCRETE.getAllValuesArray(new BlockItemDescriptable[0]));
		...
		tag(VoltaicTags.Items.INSULATES_PLAYER_FEET).add(ElectrodynamicsItems.ITEM_RUBBERBOOTS.get(), ElectrodynamicsItems.ITEM_COMPOSITEBOOTS.get());
