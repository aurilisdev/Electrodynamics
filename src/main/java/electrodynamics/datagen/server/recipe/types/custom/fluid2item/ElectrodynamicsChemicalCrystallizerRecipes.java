...
public class ElectrodynamicsChemicalCrystallizerRecipes extends AbstractRecipeGenerator {
	...
	for (SubtypeSulfateFluid fluid : SubtypeSulfateFluid.values()) {
		...
		newRecipe(new ItemStack(fluid.crystal.get()), 0, 200, 800.0,
			"crystal_" + fluid.name() + "_from_sulfate")
			//
	...
	return FinishedRecipeItemOutput
		.of(ElectrodynamicsRecipies.CHEMICAL_CRYSTALIZER_SERIALIZER.get(), stack, xp, ticks, usagePerTick)
		.name(RecipeCategory.FLUID_2_ITEM, modID, "chemical_crystallizer/" + name);
    }
