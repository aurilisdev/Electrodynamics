...
public class ElectrodynamicsElectrolyticSeparatorRecipes extends AbstractRecipeGenerator {
	...
	newRecipe(
		new GasStack(ElectrodynamicsGases.OXYGEN.get(), 1000, Gas.ROOM_TEMPERATURE, Gas.PRESSURE_AT_SEA_LEVEL),
		0, 200, 250.0, "water_to_hydrogen_and_oxygen")
		//
		.addFluidTagInput(FluidTags.WATER, 1000)
		//
		.addGasBiproduct(new ProbableGas(new GasStack(ElectrodynamicsGases.HYDROGEN.get(), 2000,
	...
	return FinishedRecipeGasOutput
		.of(ElectrodynamicsRecipies.ELECTROLYTIC_SEPARATOR_SERIALIZER.get(), stack, xp, ticks, usagePerTick)
		.name(RecipeCategory.FLUID_2_GAS, modID, "electrolytic_separator/" + name);
    }
