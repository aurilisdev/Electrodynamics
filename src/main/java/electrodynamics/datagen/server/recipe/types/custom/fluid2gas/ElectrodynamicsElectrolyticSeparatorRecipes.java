...
public class ElectrodynamicsElectrolyticSeparatorRecipes extends AbstractRecipeGenerator {
	...
	newRecipe(new GasStack(ElectrodynamicsGases.OXYGEN.value(), 1, Gas.ROOM_TEMPERATURE, Gas.PRESSURE_AT_SEA_LEVEL),
		0, 1, 250.0, "water_to_hydrogen_and_oxygen", modID)
		//
		.addFluidTagInput(FluidTags.WATER, 1)
		//
		.addGasBiproduct(new ProbableGas(new GasStack(ElectrodynamicsGases.HYDROGEN.value(), 2,
			Gas.ROOM_TEMPERATURE, Gas.PRESSURE_AT_SEA_LEVEL), 1))
		//
		.save(output);

    }

    public Fluid2GasBuilder<ElectrolyticSeparatorRecipe> newRecipe(GasStack stack, float xp, int ticks,
	    double usagePerTick, String name, String group) {
	return new Fluid2GasBuilder<>(ElectrolyticSeparatorRecipe::new, stack,
		BaseRecipeBuilder.RecipeCategory.FLUID_2_GAS, modID, "electrolytic_separator/" + name, group, xp, ticks,
		usagePerTick);
    }
