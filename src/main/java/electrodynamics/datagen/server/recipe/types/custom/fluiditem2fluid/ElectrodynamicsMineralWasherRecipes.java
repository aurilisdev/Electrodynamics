...
public class ElectrodynamicsMineralWasherRecipes extends AbstractRecipeGenerator {
	...
	public void addRecipes(Consumer<IFinishedRecipe> consumer) {
		...
		for (SubtypeSulfateFluid fluid : SubtypeSulfateFluid.values()) {
			if (fluid.source != null) {
				newRecipe(new FluidStack(ElectrodynamicsFluids.FLUIDS_SULFATE.getValue(fluid), 1000), 0, 200, 400.0, "sulfate_" + fluid.name())
						//
	...
	public FinishedRecipeFluidOutput newRecipe(FluidStack stack, float xp, int ticks, double usagePerTick, String name) {
		return FinishedRecipeFluidOutput.of(ElectrodynamicsRecipies.MINERAL_WASHER_SERIALIZER.get(), stack, xp, ticks, usagePerTick).name(RecipeCategory.FLUID_ITEM_2_FLUID, modID, "mineral_washer/" + name);
	}
