...
public class ElectrodynamicsLangKeyProvider extends BaseLangKeyProvider {
    ...
    public ElectrodynamicsLangKeyProvider(PackOutput output, Locale locale) {
	super(output, locale, Electrodynamics.ID);
    }
	...
	default:
	    ...
	    addGuidebook("chapter.tools.kineticl1",
		    "To use the Kinetic Railgun, hold the gun in one hand and the ammo type of your choice in the other. The Kinetic Railgun generates %1$s Joules of heat per shot.");
    ...
    private String wireDisplayName(SubtypeWire wire) {
	SubtypeWire.WireClass wireClass = (SubtypeWire.WireClass) wire.getWireClass();
	String material = toTitleCase(((SubtypeWire.WireMaterial) wire.getWireMaterial()).name());
	if (wireClass == SubtypeWire.WireClass.BARE) {
	    return material + " Wire";
	}
	String color = wireColorDisplayName((SubtypeWire.WireColor) wire.getWireColor());
	String prefix = switch (wireClass) {
	case INSULATED -> "Insulated";
	case THICK -> "Thick";
	case CERAMIC -> "Ceramic";
	case LOGISTICAL -> "Logistical";
	default -> toTitleCase(wireClass.name());
	};
	return prefix + " " + material + " Wire (" + color + ")";
    }

    private String wireColorDisplayName(SubtypeWire.WireColor color) {
	return switch (color) {
	case LIGHTBLUE -> "Light Blue";
	case LIGHTGRAY -> "Light Gray";
	default -> toTitleCase(color.name());
	};
    }
