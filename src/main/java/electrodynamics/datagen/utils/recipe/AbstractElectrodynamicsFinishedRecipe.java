...
public abstract class AbstractElectrodynamicsFinishedRecipe implements IFinishedRecipe {
	...
	protected AbstractElectrodynamicsFinishedRecipe(IRecipeSerializer<?> serializer, double experience, int processTime, double usagePerTick) {
	...
	public AbstractElectrodynamicsFinishedRecipe addItemTagInput(INamedTag<Item> tag, int count) {
	...
	public AbstractElectrodynamicsFinishedRecipe addFluidTagInput(INamedTag<Fluid> tag, int count) {
	...
	public void serializeRecipeData(JsonObject recipeJson) {
		...
		if (itemInputsCount > 0) {
			...
			for (Pair<INamedTag<Item>, Integer> itemTags : tagItemIngredients) {
				itemJson = new JsonObject();
				itemJson.addProperty("tag", itemTags.getFirst().getName().toString());
		...
		if (fluidInputsCount > 0) {
			...
			for (Pair<INamedTag<Fluid>, Integer> fluidTags : tagFluidIngredients) {
				fluidJson = new JsonObject();
				fluidJson.addProperty("tag", fluidTags.getFirst().getName().toString());
