...
public abstract class ElectrodynamicsRecipe implements IRecipe<RecipeWrapper> {
	...
	public ElectrodynamicsRecipe(ResourceLocation recipeID, double experience, int ticks, double usagePerTick, ProbableItem[] itemBiproducts, ProbableFluid[] fluidBiproducts) {
		id = recipeID;
		xp = experience;
		this.ticks = ticks;
	...
	public static List<ElectrodynamicsRecipe> findRecipesbyType(IRecipeType<? extends ElectrodynamicsRecipe> typeIn, World world) {
		return world != null ? world.getRecipeManager().getAllRecipesFor((IRecipeType<ElectrodynamicsRecipe>) typeIn) : Collections.emptyList();
	}
