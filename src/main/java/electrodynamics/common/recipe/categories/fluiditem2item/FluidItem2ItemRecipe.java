	...
	public boolean matchesRecipe(ComponentProcessor pr) {
		if(this.ITEM_INPUT.testStack(pr.getInput())) {
			...
			for(int i = 0; i < inputFluids.size();i++) {
				...
				if((tank != null) && tank.getFluid().getFluid().isEquivalentTo(this.FLUID_INPUT.getFluidStack().getFluid())
						&& (tank.getFluidAmount() >= this.FLUID_INPUT.getFluidStack().getAmount())) {
					return true;
				}
			}
		}
