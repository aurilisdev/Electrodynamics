...
public class FluidItem2GasRecipeSerializer<T extends FluidItem2GasRecipe> extends ElectrodynamicsRecipeSerializer<T> {
	...
	public FluidItem2GasRecipeSerializer(Class<T> recipeClass) {
		super(recipeClass);
	}
	...
	public T fromJson(ResourceLocation recipeId, JsonObject recipeJson) {
		...
		try {
			Constructor<T> recipeConstructor = getRecipeClass().getDeclaredConstructor(ResourceLocation.class, CountableIngredient[].class, FluidIngredient[].class, GasStack.class, double.class, int.class, double.class, ProbableItem[].class, ProbableFluid[].class, ProbableGas[].class);
