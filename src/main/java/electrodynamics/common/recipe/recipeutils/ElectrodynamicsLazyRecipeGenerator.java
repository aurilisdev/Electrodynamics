...
public class ElectrodynamicsLazyRecipeGenerator {
	...
	public static void o2ORecipes() {
		...
		for (SubtypeIngot from : SubtypeIngot.values()) {
			...
			for (SubtypeWire to : SubtypeWire.values()) {
				if (from.name().equals(to.name())) {
					WIRE_MILL_RECIPES.add(new String[] { to.name() + "_wire_from_" + from.name() + "_ingot",
			...
			for (SubtypeDust to : SubtypeDust.values()) {
				if (from.name().equals(to.name())) {
					MINERAL_GRINDER_RECIPES.add(new String[] { to.name() + "_dust_from_" + from.name() + "_ingot",
			...
			for (SubtypePlate to : SubtypePlate.values()) {
				if (from.name().equals(to.name())) {
					MINERAL_CRUSHER_RECIPES.add(new String[] { to.name() + "_plate_from_" + from.name() + "_ingot",
		...
		for (SubtypeDust to : SubtypeDust.values()) {
			...
			for (SubtypeOre from : SubtypeOre.values()) {
				if (to.name().equals(from.name().replace("vanadinite", "vanadium"))) {
					MINERAL_GRINDER_RECIPES.add(new String[] { to.name() + "_dust_from_" + from.name() + "_ore", getO2ORecipe(mineralGrinderRecipeID,
							"forge:" + from.forgeTag().replace("vanadinite", "vanadium"), true, 1, "electrodynamics:dust" + to.name(), 2) });
				}
			}
			...
			for (SubtypeImpureDust from : SubtypeImpureDust.values()) {
				if (to.name().equals(from.name().replace("vanadinite", "vanadium"))) {
					MINERAL_GRINDER_RECIPES
							.add(new String[] { to.name() + "_dust_from_imp" + from.name() + "_dust", getO2ORecipe(mineralGrinderRecipeID,
									"electrodynamics:impuredust" + from.name(), false, 1, "electrodynamics:dust" + to.name(), 1) });
				}
			}
		}

		// Ore -> Dust
		MINERAL_GRINDER_RECIPES.add(new String[] { SubtypeDust.sulfur.name() + "_dust_from_" + SubtypeOre.sulfur.name() + "_ore",
				getO2ORecipe(mineralGrinderRecipeID, SubtypeOre.sulfur.forgeTag(), true, 1, "electrodynamics:dust" + SubtypeDust.sulfur.name(), 1) });

		MINERAL_GRINDER_RECIPES.add(new String[] { SubtypeDust.gold.name() + "_dust_from_gold_ore",
				getO2ORecipe(mineralGrinderRecipeID, "forge:ores/gold", true, 1, "electrodynamics:dust" + SubtypeDust.gold.name(), 2) });

		MINERAL_GRINDER_RECIPES.add(new String[] { SubtypeDust.iron.name() + "_dust_from_iron_ore",
				getO2ORecipe(mineralGrinderRecipeID, "forge:ores/iron", true, 1, "electrodynamics:dust" + SubtypeDust.iron.name(), 2) });

		// Ore -> Gem
		MINERAL_GRINDER_RECIPES.add(
				new String[] { "coal_gem_from_coal_ore", getO2ORecipe(mineralGrinderRecipeID, "forge:ores/coal", true, 1, "minecraft:coal", 2) });

		MINERAL_GRINDER_RECIPES.add(new String[] { "lapis_gem_from_lapis_ore",
				getO2ORecipe(mineralGrinderRecipeID, "forge:ores/lapis", true, 1, "minecraft:lapis_lazuli", 9) });

		MINERAL_GRINDER_RECIPES.add(new String[] { "diamond_gem_from_diamond_ore",
				getO2ORecipe(mineralGrinderRecipeID, "forge:ores/diamond", true, 1, "minecraft:diamond", 2) });

		MINERAL_GRINDER_RECIPES.add(new String[] { "redstone_gem_from_redstone_ore",
		...
		for (SubtypeImpureDust to : SubtypeImpureDust.values()) {
			...
			for (SubtypeOre from : SubtypeOre.values()) {
				if (to.name().equals(from.name().replace("vanadinite", "vanadium"))) {
					MINERAL_CRUSHER_RECIPES.add(new String[] { "imp_" + to.name() + "_dust_from_" + from.name() + "_ore",
							getO2ORecipe(mineralCrusherRecipeID, "forge:" + from.forgeTag().replace("vanadinite", "vanadium"), true, 1,
									"electrodynamics:impuredust" + to.name(), 3) });
				}
			}
		}

		// Ore -> Impure Dust
		MINERAL_CRUSHER_RECIPES.add(new String[] { "imp_" + SubtypeImpureDust.gold.name() + "_dust_from_gold_ore",
				getO2ORecipe(mineralCrusherRecipeID, "forge:ores/gold", true, 1, "electrodynamics:impuredust" + SubtypeImpureDust.gold.name(), 3) });

		MINERAL_CRUSHER_RECIPES.add(new String[] { "imp_" + SubtypeImpureDust.iron.name() + "_dust_from_iron_ore",
		...
		for (SubtypeCrystal from : SubtypeCrystal.values()) {
			...
			for (SubtypeImpureDust to : SubtypeImpureDust.values()) {
				if (to.name().equals(from.name().replace("vanadinite", "vanadium"))) {
					MINERAL_CRUSHER_RECIPES
							.add(new String[] { "imp_" + to.name() + "_dust_from_" + from.name() + "_cryst", getO2ORecipe(mineralCrusherRecipeID,
									"electrodynamics:crystal" + from.name(), false, 1, "electrodynamics:impuredust" + to.name(), 1) });
				}
			}
		}
		...
		MINERAL_CRUSHER_RECIPES
				.add(new String[] { "flint_from_gravel", getO2ORecipe(mineralCrusherRecipeID, "forge:gravel", true, 1, "minecraft:flint", 1) });

		MINERAL_CRUSHER_RECIPES.add(new String[] { SubtypeDust.obsidian.name() + "_dust_from_obsidian",
		...
		MINERAL_GRINDER_RECIPES.add(new String[] { SubtypeDust.endereye.name() + "_from_ender_eye",
				getO2ORecipe(mineralGrinderRecipeID, "minecraft:ender_eye", false, 1, "electrodynamics:dust" + SubtypeDust.endereye.name(), 1) });

		MINERAL_GRINDER_RECIPES
				.add(new String[] { "sand_from_gravel", getO2ORecipe(mineralGrinderRecipeID, "forge:gravel", true, 1, "minecraft:sand", 1) });

		MINERAL_GRINDER_RECIPES.add(new String[] { "gravel_from_cobblestone",
				getO2ORecipe(mineralGrinderRecipeID, "forge:cobblestone", true, 1, "minecraft:gravel", 1) });

		MINERAL_GRINDER_RECIPES.add(
				new String[] { "cobblestone_from_stone", getO2ORecipe(mineralGrinderRecipeID, "forge:stone", true, 1, "minecraft:cobblestone", 1) });
	...
	public static void dO2ORecipes() {
		...
		OXIDATION_FURNACE_RECIPES.add(new String[] { SubtypeOxide.disulfur.name() + "_oxide",
				getDO2ORecipe(oxidationFurnaceRecipeID, "forge:" + SubtypeDust.sulfur.forgeTag(), true, 1, "minecraft:coals", true, 1,
						"electrodynamics:oxide" + SubtypeOxide.disulfur.name(), 1) });

		OXIDATION_FURNACE_RECIPES.add(new String[] { SubtypeOxide.vanadium.name() + "_oxide",
				getDO2ORecipe(oxidationFurnaceRecipeID, "forge:" + SubtypeDust.vanadium.forgeTag(), true, 1, "minecraft:coals", true, 1,
						"electrodynamics:oxide" + SubtypeOxide.vanadium.name(), 1) });

		OXIDATION_FURNACE_RECIPES.add(new String[] { SubtypeOxide.trisulfur.name() + "_oxide",
				getDO2ORecipe(oxidationFurnaceRecipeID, "forge:" + SubtypeOxide.vanadium.forgeTag(), true, 1,
						"forge:" + SubtypeOxide.disulfur.forgeTag(), true, 1, "electrodynamics:oxide" + SubtypeOxide.trisulfur.name(), 1) });
	...
	public static void fluidItem2FluidRecipes() {
		...
		CHEMICAL_MIXER_RECIPES.add(new String[] { "sulfuric_acid", getFluidItem2FluidRecipe(chemicalMixerRecipeID, "forge:oxide/trisulfur", true, 1,
				"minecraft:water", 1000, "electrodynamics:fluidsulfuricacid", 2500) });

		FERMENTATION_PLANT_RECIPES.add(new String[] { "ethanol_from_seeds", getFluidItem2FluidRecipe(fermentationPlantRecipeId, "forge:seeds", true,
				9, "minecraft:water", 1000, "electrodynamics:fluidethanol", 100) });

		FERMENTATION_PLANT_RECIPES.add(new String[] { "ethanol_from_carrots", getFluidItem2FluidRecipe(fermentationPlantRecipeId,
				"forge:crops/carrot", true, 12, "minecraft:water", 1000, "electrodynamics:fluidethanol", 100) });

		FERMENTATION_PLANT_RECIPES.add(new String[] { "ethanol_from_mushrooms", getFluidItem2FluidRecipe(fermentationPlantRecipeId, "forge:mushrooms",
				true, 11, "minecraft:water", 1000, "electrodynamics:fluidethanol", 100) });

		FERMENTATION_PLANT_RECIPES.add(new String[] { "ethanol_from_potatos", getFluidItem2FluidRecipe(fermentationPlantRecipeId,
				"forge:crops/potato", true, 13, "minecraft:water", 1000, "electrodynamics:fluidethanol", 100) });

		FERMENTATION_PLANT_RECIPES.add(new String[] { "ethanol_from_wheat", getFluidItem2FluidRecipe(fermentationPlantRecipeId, "forge:crops/wheat",
				true, 13, "minecraft:water", 1000, "electrodynamics:fluidethanol", 100) });

		FERMENTATION_PLANT_RECIPES.add(new String[] { "ethanol_from_pumpkins", getFluidItem2FluidRecipe(fermentationPlantRecipeId,
				"minecraft:pumpkin", false, 25, "minecraft:water", 1000, "electrodynamics:fluidethanol", 100) });

		FERMENTATION_PLANT_RECIPES.add(new String[] { "ethanol_from_melon_slices", getFluidItem2FluidRecipe(fermentationPlantRecipeId,
				"minecraft:melon_slice", false, 3, "minecraft:water", 1000, "electrodynamics:fluidethanol", 100) });

		FERMENTATION_PLANT_RECIPES.add(new String[] { "ethanol_from_sugar_cane", getFluidItem2FluidRecipe(fermentationPlantRecipeId,
				"minecraft:sugar_cane", false, 15, "minecraft:water", 1000, "electrodynamics:fluidethanol", 100) });
