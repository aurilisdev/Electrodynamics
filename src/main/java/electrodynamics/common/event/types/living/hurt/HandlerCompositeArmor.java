...
public class HandlerCompositeArmor extends AbstractLivingHurtHandler {
  ...
  private static final ItemStack[] COMPOSITE_ARMOR =
      new ItemStack[] {new ItemStack(ElectrodynamicsItems.ITEM_COMPOSITEHELMET.get()),
  ...
  public void handle(LivingHurtEvent event) {
    ...
    if (compareArmor(armorPieces, COMPOSITE_ARMOR) || compareArmor(armorPieces, COMBAT_ARMOR)) {
      ...
      if (event.getAmount() >= LETHAL_DAMAGE_AMOUNT && stored > 0) {
        ...
        event.getEntity().getCommandSenderWorld().playSound(null, event.getEntity().blockPosition(),
            ElectrodynamicsSounds.SOUND_CERAMICPLATEBREAKING.get(), SoundSource.PLAYERS, 1, 1);
      }
    }
  ...
  private boolean compareArmor(List<ItemStack> set1, ItemStack[] set2) {
    if (set1.size() >= 3) {
      return set1.get(0).getItem() == set2[3].getItem()
          && set1.get(1).getItem() == set2[2].getItem()
          && set1.get(2).getItem() == set2[1].getItem()
          && set1.get(3).getItem() == set2[0].getItem();
    }
    return false;
  }
