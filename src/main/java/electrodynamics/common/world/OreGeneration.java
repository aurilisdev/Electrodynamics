			...
			for (Direction direction : Direction.values()) {
				...
				if (random.nextFloat() < 0.3) {
					if (level.getBlockState(offset).getBlock() == Blocks.STONE) {
						level.setBlock(offset, UnifiedElectrodynamicsRegister.getSafeBlock(SubtypeOre.sulfur).defaultBlockState(), 3);
					}
					if (level.getBlockState(offset).getBlock() == Blocks.DEEPSLATE) {
						level.setBlock(offset, UnifiedElectrodynamicsRegister.getSafeBlock(SubtypeOreDeepslate.sulfur).defaultBlockState(), 3);
					}
				}
			}
		}
