...
public class ElectricNetwork extends AbstractNetwork<GenericTileWire, IWire, TransferPack, ElectricNetwork>
	...
	for (BlockEntity receiver : availableAcceptors) {
	    TransferPack dedicated = TransferPack.joulesVoltage(
		    maxTransfer.getJoules() * (usage.get(receiver) / totalUsage), maxTransfer.getVoltage());
	    HashMap<Direction, TransferPack> perConnectionMap;
	    if (acceptorInputMap.containsKey(receiver)) {
		TransferPack perConnection = TransferPack.joulesVoltage(
			dedicated.getJoules() / acceptorInputMap.get(receiver).size(), maxTransfer.getVoltage());
	...
	for (Map.Entry<Long, HashSet<GenericTileWire>> entry : ampacityToWireMap.entrySet()) {
	    if (entry.getKey() <= transmittedLastTick / voltage * 20
		    && entry.getKey() <= transmittedThisTick / voltage * 20) {
	...
	ICapabilityElectrodynamic electro = reciever
	...
	if (maxTransferBuffer > 0) {
		...
		if (resistance > 0) {
		    ...
		    double maxWatts = (-voltage * voltage
			    + voltage * Math.sqrt(voltage * voltage + 4.0 * bufferAsWatts * resistance))
			    / (2.0 * resistance);
		    ...
		    transferBuffer -= sendToReceivers(TransferPack.joulesVoltage(transferBuffer, voltage),
			    producersList, false).getJoules();
		}
	    }
	} else {
	    transferBuffer = 0;
	...
	maxTransferBuffer = getConnectedLoad(
		new LoadProfile(TransferPack.joulesVoltage(transmittedLastTick, lastVoltage),
			TransferPack.joulesVoltage(transmittedLastTick, lastVoltage)),
		Direction.UP).getJoules();

	/*
	 * 
	 * Iterator<GenericTileWire> it = conductorSet.iterator(); boolean broken =
	 * false; while (it.hasNext()) { GenericTileWire conductor = it.next(); if
	 * (conductor instanceof BlockEntity entity && entity.isRemoved() ||
	 * conductor.getNetwork() != this) { broken = true; break; } } if (broken) {
	 * refreshNewNetwork(); }
	 * 
	 */
	if (getSize() == 0) {
	...
	for (BlockEntity tile : load) {
		...
		ICapabilityElectrodynamic electro = tile
			.getCapability(VoltaicCapabilities.CAPABILITY_ELECTRODYNAMIC_BLOCK, direction)
			.orElse(CapabilityUtils.EMPTY_ELECTRO);

		if (electro == CapabilityUtils.EMPTY_ELECTRO) {

		    IEnergyStorage fe = tile.getCapability(ForgeCapabilities.ENERGY, direction)
			    .orElse(CapabilityUtils.EMPTY_FE);

		    if (fe == CapabilityUtils.EMPTY_FE) {
			capLoad = TransferPack.EMPTY;
		    } else {
			capLoad = TransferPack.joulesVoltage(fe.receiveEnergy(Integer.MAX_VALUE, true),
		...
		if (capLoad.getJoules() != 0) {
		    ...
		    connectedLoad = TransferPack.joulesVoltage(connectedLoad.getJoules() + capLoad.getJoules(),
			    Math.max(connectedLoad.getVoltage(), capLoad.getVoltage()));
		    break;
