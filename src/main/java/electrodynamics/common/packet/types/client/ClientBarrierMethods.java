...
public class ClientBarrierMethods {
    ...
    /*
     * public static void handlePropertiesUpdateClient(BlockPos pos,
     * HashSet<PropertyWrapper> values) { ClientLevel world =
     * Minecraft.getInstance().level; if (world == null) { return; } BlockEntity
     * tile = world.getBlockEntity(pos);
     * 
     * if (tile instanceof IPropertyHolderTile holder) { for (PropertyWrapper
     * wrapper : values) {
     * 
    ...
    public static void handlerClientGasCollectorCards(
	    HashMap<Item, GasCollectorChromoCardsRegister.AtmosphericResult> results) {
	GasCollectorChromoCardsRegister.INSTANCE.setClientValues(results);
    }

    /*
     * public static void handleUpdateSpecificPropertyClient(PropertyWrapper
     * wrapper, BlockPos pos){ ClientLevel world = Minecraft.getInstance().level; if
     * (world == null) { return; } BlockEntity tile = world.getBlockEntity(pos); if
     * (tile instanceof IPropertyHolderTile holder) {
     * holder.getPropertyManager().update(wrapper.index(), wrapper.value()); } }
     * 
     */
