package electrodynamics.common.item.gear.armor.types.nvgoggles;
...
public class ItemNightVisionGoggles extends ArmorItem implements IItemElectric {
			...
			if(nvgs.getJoulesStored(nvgStack) >= JOULES_PER_TICK) {
				if(nvgStack.equals(player.getItemBySlot(EquipmentSlot.HEAD), false)) {
					...
					if(player.hasEffect(MobEffects.NIGHT_VISION)) {
						player.getEffect(MobEffects.NIGHT_VISION).update(new MobEffectInstance(MobEffects.NIGHT_VISION, DURATION_SECONDS * 20, 0, true, true, true));
					} else {
	...
	public String getArmorTexture(ItemStack stack, Entity entity, EquipmentSlot slot, String type) {
		boolean isOn = stack.getCapability(ElectrodynamicsCapabilities.BOOLEAN_STORAGE_CAPABILITY).map(m -> m.getBoolean()).orElse(false);
		if(isOn) {
