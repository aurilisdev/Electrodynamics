...
public class BlockOre extends DropExperienceBlock {
	...
	super(Properties.copy(Blocks.STONE).requiresCorrectToolForDrops().strength(ore.hardness, ore.resistance)
		.randomTicks(), UniformInt.of(ore.minXP, ore.maxXP));
	this.ore = ore;
	deepOre = null;
    }

    public BlockOre(SubtypeOreDeepslate ore) {
	super(Properties.copy(Blocks.STONE).sound(SoundType.DEEPSLATE).requiresCorrectToolForDrops()
		.strength(ore.hardness + 1.5f, ore.resistance + 1.5f).randomTicks(),
		UniformInt.of(ore.minXP, ore.maxXP));
	deepOre = ore;
	this.ore = null;
    }
	...
	if (!VoltaicConstants.ORES_EMIT_RADIATION
		|| level.getLevelData().getGameTime() % VoltaicConstants.ORE_RADIATION_ADMIT_RATE != 0) {
	    return;
	}
	...
	for (Entity entity : level.getAllEntities()) {
	    ...
	    double recieved = RadiationManager.getAppliedRadiation(level, pos, entity.getOnPos().above(), rad.amount(),
		    rad.strength());
