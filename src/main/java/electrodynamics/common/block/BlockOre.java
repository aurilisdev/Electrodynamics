...
public class BlockOre extends DropExperienceBlock {
	...
	super(UniformInt.of(ore.minXP, ore.maxXP), Properties.ofFullCopy(Blocks.STONE).requiresCorrectToolForDrops()
		.strength(ore.hardness, ore.resistance).randomTicks());
	this.ore = ore;
	deepOre = null;
    }

    public BlockOre(SubtypeOreDeepslate ore) {
	super(UniformInt.of(ore.minXP, ore.maxXP), Properties.ofFullCopy(Blocks.STONE).sound(SoundType.DEEPSLATE)
		.requiresCorrectToolForDrops().strength(ore.hardness + 1.5f, ore.resistance + 1.5f).randomTicks());
	deepOre = ore;
	this.ore = null;
    }
