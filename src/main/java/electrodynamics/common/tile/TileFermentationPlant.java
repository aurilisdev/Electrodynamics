...
public class TileFermentationPlant extends GenericTileTicking {
    ...
    public TileFermentationPlant() {
		...
		addComponent(new ComponentProcessor(this)
				.upgradeSlots(2, 3, 4)
				.usage(Constants.FERMENTATIONPLANT_USAGE_PER_TICK)
				.type(ComponentProcessorType.ObjectToObject)
				.canProcess(component -> canProcessFermPlan(component))
    ...
    protected boolean canProcessFermPlan(ComponentProcessor processor) {
		...
		boolean canProcess = processor.canProcessFluidItem2FluidRecipe(processor, FluidItem2FluidRecipe.class, ElectrodynamicsRecipeInit.FERMENTATION_PLANT_TYPE);
		return canProcess;
    }
