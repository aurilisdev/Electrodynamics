...
public class TileOxidationFurnace extends GenericTile implements ITickableSound {
	...
	public TileOxidationFurnace() {
		super(ElectrodynamicsBlockTypes.TILE_OXIDATIONFURNACE.get());
	...
	protected boolean canProcessOxideFurn(ComponentProcessor component) {
		boolean canProcess = component.canProcessItem2ItemRecipe(component, ElectrodynamicsRecipeInit.OXIDATION_FURNACE_TYPE);
		if (BlockEntityUtils.isLit(this) ^ canProcess) {
			BlockEntityUtils.updateLit(this, canProcess);
		}
		
		if (getBlockState().is(ElectrodynamicsBlocks.getBlock(SubtypeMachine.oxidationfurnacerunning))) {
			getLevel().setBlockAndUpdate(getBlockPos(), ElectrodynamicsBlocks.getBlock(SubtypeMachine.oxidationfurnace).defaultBlockState().setValue(GenericEntityBlock.FACING, getFacing()));
		}
