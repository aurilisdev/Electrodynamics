...
public class TileReinforcedAlloyer extends GenericTile implements ITickableSound {
	...
	addComponent(new ComponentInventory(this,
		ComponentInventory.InventoryBuilder.newInv().processors(1, 2, 1, 1).upgrades(3))
	...
	if (level.random.nextDouble() < 0.2) {
	    ...
	    float scale = 0.1F * (level.random.nextFloat() * 0.5F + 0.5F) * 2.0F
		    * (level.random.nextFloat() * 2.0F + 0.2F);
	    ...
	    level.addParticle(new ParticleOptionLavaWithPhysics().setParameters(scale, lifetime, 1),
