...
public class TileReinforcedAlloyer extends GenericTile implements ITickableSound {
	...
	public TileReinforcedAlloyer() {
		super(ElectrodynamicsBlockTypes.TILE_REINFORCEDALLOYER.get());
	...
	protected boolean canProcessReinfAlloy(ComponentProcessor component) {
		boolean canProcess = component.canProcessItem2ItemRecipe(component, ElectrodynamicsRecipeInit.REINFORCED_ALLOYER_TYPE);
		if (BlockEntityUtils.isLit(this) ^ canProcess) {
			BlockEntityUtils.updateLit(this, canProcess);
		}
		
		if (getBlockState().is(ElectrodynamicsBlocks.getBlock(SubtypeMachine.reinforcedalloyerrunning))) {
			getLevel().setBlockAndUpdate(getBlockPos(), ElectrodynamicsBlocks.getBlock(SubtypeMachine.reinforcedalloyer).defaultBlockState().setValue(GenericEntityBlock.FACING, getFacing()));
		}
