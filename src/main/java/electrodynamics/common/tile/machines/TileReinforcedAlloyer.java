...
public class TileReinforcedAlloyer extends GenericTile implements ITickableSound {

	private boolean isSoundPlaying = false;

	public TileReinforcedAlloyer() {
		super(ElectrodynamicsTiles.TILE_REINFORCEDALLOYER.get());
		addComponent(new ComponentPacketHandler(this));
		addComponent(new ComponentTickable(this).tickClient(this::tickClient));
		addComponent(new ComponentElectrodynamic(this, false, true).setInputDirections(BlockEntityUtils.MachineDirection.BACK).voltage(VoltaicCapabilities.DEFAULT_VOLTAGE * 8));
		...
		addComponent(new ComponentContainerProvider(SubtypeMachine.reinforcedalloyer.tag(), this).createMenu((id, player) -> new ContainerDO2OProcessor(id, player, getComponent(IComponentType.Inventory), getCoordsArray())));
		addComponent(new ComponentProcessor(this).canProcess(this::canProcessReinfAlloy).process(ComponentProcessor::processItem2ItemRecipe));
	}

	protected boolean canProcessReinfAlloy(ComponentProcessor component, int procNumber) {
		boolean canProcess = component.canProcessItem2ItemRecipe(procNumber, ElectrodynamicsRecipies.REINFORCED_ALLOYER_TYPE);
	...
	protected void tickClient(ComponentTickable tickable) {
		...
		if (level.random.nextDouble() < 0.2) {
			...
			double axisShift = Math.min(Voltaic.RANDOM.nextDouble(), 0.63) + 0.185;
