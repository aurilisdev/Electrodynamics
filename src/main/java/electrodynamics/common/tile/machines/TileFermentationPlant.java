...
public class TileFermentationPlant extends GenericMaterialTile {
    ...
    public TileFermentationPlant() {
        super(ElectrodynamicsTiles.TILE_FERMENTATIONPLANT.get());
        ...
        addComponent(new ComponentFluidHandlerMulti(this).setTanks(1, 1, new int[] { MAX_TANK_CAPACITY }, new int[] { MAX_TANK_CAPACITY }).setInputDirections(BlockEntityUtils.MachineDirection.LEFT).setOutputDirections(BlockEntityUtils.MachineDirection.RIGHT).setRecipeType(ElectrodynamicsRecipies.FERMENTATION_PLANT_TYPE));
        ...
        addComponent(new ComponentProcessor(this).canProcess((processor, procNumber) -> processor.outputToFluidPipe().consumeBucket().dispenseBucket().canProcessFluidItem2FluidRecipe(procNumber, ElectrodynamicsRecipies.FERMENTATION_PLANT_TYPE)).process(ComponentProcessor::processFluidItem2FluidRecipe));
