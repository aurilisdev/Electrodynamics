...
public class TileEnergizedAlloyer extends GenericTile implements ITickableSound {
	...
	public TileEnergizedAlloyer() {
		super(ElectrodynamicsBlockTypes.TILE_ENERGIZEDALLOYER.get());
	...
	protected boolean canProcessEnergAlloy(ComponentProcessor component) {
		boolean canProcess = component.canProcessItem2ItemRecipe(component, ElectrodynamicsRecipeInit.ENERGIZED_ALLOYER_TYPE);
		if (BlockEntityUtils.isLit(this) ^ canProcess) {
			BlockEntityUtils.updateLit(this, canProcess);
		}

		if (getBlockState().is(ElectrodynamicsBlocks.getBlock(SubtypeMachine.energizedalloyerrunning))) {
			getLevel().setBlockAndUpdate(getBlockPos(), ElectrodynamicsBlocks.getBlock(SubtypeMachine.energizedalloyer).defaultBlockState().setValue(GenericEntityBlock.FACING, getFacing()));
		}
