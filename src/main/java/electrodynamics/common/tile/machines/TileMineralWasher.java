...
public class TileMineralWasher extends GenericMaterialTile {
    ...
    public TileMineralWasher(BlockPos worldPosition, BlockState blockState) {
        ...
        addComponent(new ComponentFluidHandlerMulti(this).setTanks(1, 1, new int[]{MAX_TANK_CAPACITY}, new int[]{MAX_TANK_CAPACITY}).setInputDirections(BlockEntityUtils.MachineDirection.RIGHT).setOutputDirections(BlockEntityUtils.MachineDirection.LEFT).setRecipeType(ElectrodynamicsRecipies.MINERAL_WASHER_TYPE.get()));
        ...
        addComponent(new ComponentProcessor(this).canProcess((component, procNumber) -> component.outputToFluidPipe().consumeBucket().dispenseBucket().canProcessFluidItem2FluidRecipe(procNumber, ElectrodynamicsRecipies.MINERAL_WASHER_TYPE.get())).process(ComponentProcessor::processFluidItem2FluidRecipe));
