...
public class TileElectrolyticSeparator extends GenericMaterialTile implements ITickableSound {
	...
	public TileElectrolyticSeparator() {
		...
		addComponent(new ComponentFluidHandlerMulti(this).setTanks(1, 2, arr(MAX_INPUT_TANK_CAPACITY), arr(MAX_OUTPUT_TANK_CAPACITY, MAX_OUTPUT_TANK_CAPACITY)).setInputDirections(BlockEntityUtils.MachineDirection.BACK).setOutputDirections(OXYGEN_DIRECTION, HYDROGEN_DIRECTION)
				.setRecipeType(ElectrodynamicsRecipies.ELECTROLYTIC_SEPERATOR_TYPE));
		...
		addComponent(new ComponentProcessor(this).canProcess((component, procNumber) -> component.consumeBucket().dispenseBucket().canProcessFluid2FluidRecipe(procNumber, ElectrodynamicsRecipies.ELECTROLYTIC_SEPERATOR_TYPE)).process(ComponentProcessor::processFluid2FluidRecipe));
