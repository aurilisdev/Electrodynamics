...
public class TileChemicalCrystallizer extends GenericMaterialTile implements ITickableSound {
	...
	public TileChemicalCrystallizer(BlockPos worldPosition, BlockState blockState) {
		...
		addComponent(new ComponentFluidHandlerMulti(this).setInputTanks(1, MAX_TANK_CAPACITY).setInputDirections(Direction.values()).setRecipeType(ElectrodynamicsRecipeInit.CHEMICAL_CRYSTALIZER_TYPE.get()));
		addComponent(new ComponentInventory(this, InventoryBuilder.newInv().processors(1, 0, 1, 0).bucketInputs(1).upgrades(3)).setSlotsForAllDirections(0).validUpgrades(ContainerChemicalCrystallizer.VALID_UPGRADES).valid(machineValidator()));
