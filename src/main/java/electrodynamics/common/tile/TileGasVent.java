...
public class TileGasVent extends GenericMaterialTile {
	...
	public TileGasVent(BlockPos worldPos, BlockState blockState) {
		...
		addComponent(new ComponentContainerProvider(SubtypeMachine.gasvent).createMenu((id, player) -> new ContainerGasVent(id, player, getComponent(ComponentType.Inventory), getCoordsArray())));
	}
