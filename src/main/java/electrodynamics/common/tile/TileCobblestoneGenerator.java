...
public class TileCobblestoneGenerator extends GenericTile {
	...
	public TileCobblestoneGenerator(BlockPos worldPos, BlockState blockState) {
		...
		addComponent(new ComponentInventory(this).size(4).outputs(1).upgrades(3).validUpgrades(ContainerCobblestoneGenerator.VALID_UPGRADES).valid(machineValidator()).relativeSlotFaces(0, Direction.values()));
