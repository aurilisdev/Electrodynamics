...
public class BlockItemWire extends BlockItemDescriptable {
    ...
    public BlockItemWire(BlockWire wire, Properties builder, RegistryObject<CreativeModeTab> creativeTab) {
	...
	tooltip.add(ElectroTextUtils.tooltip("itemwire.resistance", ChatFormatter
		.getChatDisplayShort(wire.wire.getResistance(), DisplayUnits.RESISTANCE).withStyle(ChatFormatting.GRAY))
		.withStyle(ChatFormatting.DARK_GRAY));
	tooltip.add(ElectroTextUtils.tooltip("itemwire.maxamps", ChatFormatter
		.getChatDisplayShort(wire.wire.getAmpacity(), DisplayUnits.AMPERE).withStyle(ChatFormatting.GRAY))
		.withStyle(ChatFormatting.DARK_GRAY));
	if (wire.wire.getInsulation().shockVoltage() == 0) {
	    ...
	    tooltip.add(
		    ElectroTextUtils
			    .tooltip("itemwire.info.insulationrating",
				    ChatFormatter.getChatDisplayShort(wire.wire.getInsulation().shockVoltage(),
