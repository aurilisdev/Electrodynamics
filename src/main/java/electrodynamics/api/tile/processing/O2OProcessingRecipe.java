	...
	public O2OProcessingRecipe(Object... objects) {
		...
		if (objects[nextStack] instanceof Subtype) {
			...
			if (objects[nextStack + 1] instanceof Integer) {
				input = new ItemStack(type.isItem() ? DeferredRegisters.SUBTYPEITEM_MAPPINGS.get(type) : DeferredRegisters.SUBTYPEBLOCK_MAPPINGS.get(type), (int) objects[nextStack + 1]);
				nextStack = 2;
			} else {
				input = new ItemStack(type.isItem() ? DeferredRegisters.SUBTYPEITEM_MAPPINGS.get(type) : DeferredRegisters.SUBTYPEBLOCK_MAPPINGS.get(type));
				nextStack = 1;
			}
