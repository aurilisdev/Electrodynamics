...
public class CapabilityCeramicPlateHandler {
	...
	if (ItemStack.areItemsEqualIgnoreDurability(armorPieces.get(0), ARMOR_PIECES[3])
		...
		if (event.getAmount() >= LETHAL_DAMAGE_AMOUNT && h.getPlateCount() > 0) {
		    ...
		    event.getEntityLiving().getEntityWorld().playSound(null, event.getEntityLiving().getPosition(),
			    SoundRegister.SOUND_CERAMICPLATEBREAKING.get(), SoundCategory.PLAYERS, 1, 1);
		}
	    });
	...
	if (ItemStack.areItemsEqualIgnoreDurability(event.getItemStack(),
	    ...
	    if (ItemStack.areItemsEqualIgnoreDurability(chestplate,
		    new ItemStack(DeferredRegisters.COMPOSITE_ARMOR_PIECES.get(EquipmentSlotType.CHEST).get()))) {
		chestplate.getCapability(CapabilityCeramicPlate.CERAMIC_PLATE_HOLDER_CAPABILITY).ifPresent(h -> {
		    if (h.getPlateCount() < 2) {
			event.getEntityLiving().getEntityWorld().playSound(event.getEntityLiving().chunkCoordX, event.getEntityLiving().chunkCoordY,
				event.getEntityLiving().chunkCoordZ, SoundRegister.SOUND_CERAMICPLATEADDED.get(), SoundCategory.PLAYERS, 1, 1, false);
			h.setPlateCount(h.getPlateCount() + 1);
