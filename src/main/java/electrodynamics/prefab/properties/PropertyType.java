...
public enum PropertyType implements IPropertyType {
	...
	Vec3(
			...
			writer -> {
				...
				net.minecraft.world.phys.Vec3 val = (net.minecraft.world.phys.Vec3) writer.value();
				...
				return new net.minecraft.world.phys.Vec3(reader.buf().readDouble(), reader.buf().readDouble(), reader.buf().readDouble());

			},

			//

			writer -> CodecUtils.VEC3_CODEC.encodeStart(NbtOps.INSTANCE, (net.minecraft.world.phys.Vec3) writer.prop().get()).result().ifPresent(tag -> writer.tag().put(writer.prop().getName(), tag)),

			//

			reader -> CodecUtils.VEC3_CODEC.decode(NbtOps.INSTANCE, reader.tag().get(reader.prop().getName())).result().get().getFirst()
