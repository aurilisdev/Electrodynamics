...
public class ComponentFluidHandlerMulti extends AbstractFluidHandler<ComponentFluidHandlerMulti> {
	...
	public ComponentFluidHandlerMulti setManualFluids(int tankCount, boolean isInput, int capacity, Fluid... fluids) {
		...
		if(isInput && inputTanks.length == 0) {
			this.inputTanks = new FluidTank[tankCount];
			for(int i = 0; i < tankCount; i++) {
				inputTanks[i] = new FluidTank(capacity);
			}
		} else if (outputTanks.length == 0) {
			this.outputTanks = new FluidTank[tankCount];
			for(int i = 0; i < tankCount; i++) {
	...
	protected void addValidFluid(Fluid fluid, boolean isInput) {
		if(isInput) {
			...
			if(!validOutputs.contains(fluid)) {
	...
	public void addFluids() {
		if (recipeType != null) {
		    ...
		    for (Recipe<?> iRecipe : recipes) {
		    	...
		    	if (hasInput) {
				    ...
				    for(FluidIngredient ing : recipe.getFluidIngredients()) {
				    	ing.getMatchingFluids().forEach(h ->{
				    		inputFluidHolder.add(h.getFluid());
				    	});
				    }   
				}
		    ...
		    setManualFluids(inTankCount, true, this.tankCapacity, inputFluidHolder.toArray(new Fluid[inputFluidHolder.size()])); 
		    setManualFluids(outTankCount, false, this.tankCapacity, outputFluidHohlder.toArray(new Fluid[inputFluidHolder.size()]));
		}
	}
}
