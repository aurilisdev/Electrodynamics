...
public class ComponentProcessor implements Component {
    ...
    /**
     * Consumes a bucket in a particular inventory slot if the container is able to accept
     * the fluid.
     * @param maxCapacity
     * @param fluids
     * @param slot
    ...
    public ComponentProcessor consumeBucket(int maxCapacity, Fluid[] fluids, int slot) {
    	ComponentInventory inv = this.holder.getComponent(ComponentType.Inventory);
    	ComponentFluidHandler tank = this.holder.getComponent(ComponentType.FluidHandler);
    ...
    /*
     * I could technically get two recipe classes with this method, but I would prefer
     * that each ReicpeType have its own method when practical, as it makes custom logic
     * and what not easier to implement long-term. Maybe once we're in the matainence stage
     * of development, we can change it.
     */
    ...
    public <T extends FluidItem2FluidRecipe> boolean canProcessFluidItem2FluidRecipe(ComponentProcessor pr, Class<T> recipeClass, IRecipeType<?> typeIn) {
    ...
    public <T extends FluidItem2ItemRecipe> boolean canProcessFluidItem2ItemRecipe(ComponentProcessor pr, Class<T> recipeClass, IRecipeType<?> typeIn) {
    	ComponentElectrodynamic electro = holder.getComponent(ComponentType.Electrodynamic);
