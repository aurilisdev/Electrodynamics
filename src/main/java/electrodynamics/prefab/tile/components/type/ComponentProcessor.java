...
public class ComponentProcessor implements Component {
		    ...
		    if (validBucket && tank.getStackFromFluid(matchingFluid).getAmount() <= maxCapacity - 1000) {
				if (isCanister) {
				    inv.setInventorySlotContents(slot,
					    new ItemStack(DeferredRegisters.SUBTYPEITEM_MAPPINGS.get(SubtypeCanister.empty)));
				} else {
    ...
    public<T extends Fluid2ItemRecipe> boolean canProcessFluid2ItemRecipe(ComponentProcessor pr, Class<T> recipeClass, IRecipeType<?> typeIn) {
    	...
    	if(recipeFound) {
    		 matchingOutputs = pr.getOutput().isEmpty() ? true : ItemStack.areItemsEqual(pr.getOutput(), recipe.getRecipeOutput());
    	}
    	//ElectrodynamicsRecipe.LOGGER.info("Recipe Found: " + recipeFound);
