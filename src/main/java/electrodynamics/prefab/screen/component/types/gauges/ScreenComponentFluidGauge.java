	...
	protected List<? extends FormattedCharSequence> getTooltips() {
		...
		if (tank != null) {
			...
			if (fluidStack.getAmount() > 0) {
				...
				tooltips.add(ElectroTextUtils.ratio(new TextComponent("0"), ChatFormatter.formatFluidMilibuckets(tank.getCapacity())).withStyle(ChatFormatting.GRAY).getVisualOrderText());
			}
		}
		return tooltips;
	}
