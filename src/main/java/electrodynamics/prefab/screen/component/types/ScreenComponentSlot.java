...
public class ScreenComponentSlot extends ScreenComponentGeneric {
	...
	public ScreenComponentSlot setHoverText(Slot slot) {
		tooltip = () -> slot.getItem().isEmpty() ? ElectroTextUtils.empty() : slot.getItem().getHoverName().copy();
		return this;
	}
	...
	public void renderForeground(MatrixStack stack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
		...
		if (isHoveredOrFocused()) {
			...
			if (tooltip != null && !tooltip.getText().getString().isEmpty()) {
				gui.displayTooltip(stack, tooltip.getText().getVisualOrderText(), xAxis, yAxis, gui.getFontRenderer());
			}
			...
			if (Screen.hasControlDown() && slot instanceof IUpgradeSlot) {
				...
				for (Item item : upgrade.getUpgrades()) {
					tooltips.add(item.getDescription().copy().withStyle(TextFormatting.GRAY).getVisualOrderText());
				}
