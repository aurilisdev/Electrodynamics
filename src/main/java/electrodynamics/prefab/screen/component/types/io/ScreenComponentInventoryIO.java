...
public class ScreenComponentInventoryIO extends ScreenComponentGeneric {
	...
	public void renderBackground(PoseStack stack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
		...
		if (uniqueColors.size() <= 16) {
			fillSquare(stack, uniqueColors, xLocation + guiWidth, yLocation + guiHeight);
			return;
		}
	...
	public void renderForeground(PoseStack stack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
		if (isVisible() && isHovered()) {
			gui.displayTooltip(stack, getLabelFromDir().getVisualOrderText(), xAxis, yAxis, gui.getFontRenderer());
		}
	}

	private void fillSquare(PoseStack stack, List<Color> colors, int x, int y) {
		...
		for (int i = 0; i < wholeRows; i++) {
			for (int j = 0; j < COLORS_PER_ROW; j++) {
				...
				fill(stack, x + 1 + j * SQUARE_SIZE, y + 1 + i * wholeHeight, x + 1 + (j + 1) * SQUARE_SIZE, y + 1 + (i + 1) * wholeHeight, colors.get(index).multiply(SLOT_GRAY).color());
		...
		for (int i = 0; i < lastRowCount - 1; i++) {
			...
			fill(stack, x + 1 + i * remainderWidth, y + 1 + wholeHeight * wholeRows, x + 1 + (i + 1) * remainderWidth, y + 1 + wholeHeight * wholeRows + remainderHeight, colors.get(index).multiply(SLOT_GRAY).color());

			index++;
		}

		fill(stack, x + 1 + remainderWidth * (lastRowCount - 1), y + 1 + wholeHeight * wholeRows, x + 1 + lastRowCount * remainderWidth + lastToAdd, y + 1 + wholeHeight * wholeRows + remainderHeight, colors.get(index).multiply(SLOT_GRAY).color());
