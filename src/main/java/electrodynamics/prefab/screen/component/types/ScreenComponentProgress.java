...
public class ScreenComponentProgress extends ScreenComponentGeneric {
	...
	public void renderBackground(MatrixStack stack, final int xAxis, final int yAxis, final int guiWidth, final int guiHeight) {
		...
		case PROGRESS_ARROW_LEFT:
			...
			blit(stack, guiWidth + this.x + on.textureWidth() - progress, guiHeight + this.y, xStart, on.textureV(), progress, on.textureHeight(), on.imageWidth(), on.imageHeight());
			break;
		case COUNTDOWN_FLAME:
			progress = (int) (progressInfoHandler.getAsDouble() * on.textureHeight());
			blit(stack, guiWidth + this.x, guiHeight + this.y + on.textureHeight() - progress, on.textureU(), on.textureV() + on.textureHeight() - progress, on.textureWidth(), progress, on.imageWidth(), on.imageHeight());
			break;
		case PROGRESS_ARROW_RIGHT: 
		case BATTERY_CHARGE_RIGHT:
		case PROGRESS_ARROW_RIGHT_BIG:
			blit(stack, guiWidth + this.x, guiHeight + this.y, on.textureU(), on.textureV(), (int) (progressInfoHandler.getAsDouble() * on.textureWidth()), on.textureHeight(), on.imageWidth(), on.imageHeight());
			break;
		default:
			break;
		}
