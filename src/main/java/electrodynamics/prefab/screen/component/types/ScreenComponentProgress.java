...
public class ScreenComponentProgress extends ScreenComponentGeneric {
	...
	public void renderBackground(PoseStack stack, final int xAxis, final int yAxis, final int guiWidth, final int guiHeight) {
		...
		case PROGRESS_ARROW_LEFT:
			...
			blit(stack, guiWidth + xLocation + on.textureWidth() - progress, guiHeight + yLocation, xStart, on.textureV(), progress, on.textureHeight(), on.imageWidth(), on.imageHeight());
			break;
		case COUNTDOWN_FLAME:
			progress = (int) (progressInfoHandler.getAsDouble() * on.textureHeight());
			blit(stack, guiWidth + xLocation, guiHeight + yLocation + on.textureHeight() - progress, on.textureU(), on.textureV() + on.textureHeight() - progress, on.textureWidth(), progress, on.imageWidth(), on.imageHeight());
			break;
		case PROGRESS_ARROW_RIGHT, BATTERY_CHARGE_RIGHT, PROGRESS_ARROW_RIGHT_BIG:
			blit(stack, guiWidth + xLocation, guiHeight + yLocation, on.textureU(), on.textureV(), (int) (progressInfoHandler.getAsDouble() * on.textureWidth()), on.textureHeight(), on.imageWidth(), on.imageHeight());
			break;
		default:
			break;
		}
