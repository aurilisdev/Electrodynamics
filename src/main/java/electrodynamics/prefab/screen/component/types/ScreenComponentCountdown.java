...
public class ScreenComponentCountdown extends AbstractScreenComponentInfo {
	...
	public void renderForeground(MatrixStack stack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
		if (isPointInRegion(this.x, this.y, xAxis, yAxis, texture.textureWidth(), texture.textureHeight())) {
	...
	private List<? extends IReorderingProcessor> getTooltips() {
		...
		if (progressInfoHandler != null) {
			tips.add(ChatFormatter.getChatDisplayShort(100 * progressInfoHandler.getAsDouble(), DisplayUnit.PERCENTAGE).withStyle(TextFormatting.GRAY).getVisualOrderText());
		}
		return tips;
	}
