...
public class ScreenComponentGasTemperature  extends ScreenComponentGuiTab {
	...
	private List<? extends FormattedCharSequence> getMaxPressureInfo() {
		...
		for (PropertyGasTank tank : handler.getInputTanks()) {
			...
			tooltips.add(ElectroTextUtils.tooltip("tankmaxin", index, ChatFormatter.getChatDisplayShort(tank.getMaxTemperature(), DisplayUnit.TEMPERATURE_KELVIN)).withStyle(ChatFormatting.GRAY).getVisualOrderText());
