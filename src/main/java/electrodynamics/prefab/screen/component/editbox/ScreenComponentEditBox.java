...
public class ScreenComponentEditBox extends ScreenComponentGeneric {
	...
	public void renderBackground(MatrixStack stack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
		...
		drawExpandedBox(stack, this.x + guiWidth, this.y + guiHeight, width, height);
		...
		if (blinkCursor) {
			if (isCursorPastLength) {
				fill(stack, textStartPreCopy, textStartY - 1, textStartPreCopy + 1, textStartY + 1 + 9, -3092272);
			} else {
	...
	public static void drawExpandedBox(MatrixStack stack, int x, int y, int boxWidth, int boxHeight) {
		if (boxWidth < 18) {
			if (boxHeight < 18) {
				blit(stack, x, y, boxWidth, boxHeight, 0, 0, boxWidth, boxHeight, boxWidth, boxHeight);
			} else {
				blit(stack, x, y, boxWidth, 7, 0, 0, boxWidth, 7, boxWidth, 18);
				...
				for (int i = 0; i < heightIterations; i++) {
					blit(stack, x, y + heightOffset, boxWidth, 4, 0, 7, boxWidth, 4, boxWidth, 18);
					heightOffset += 4;
				}
				blit(stack, x, y + heightOffset, boxWidth, remainderHeight, 0, 7, boxWidth, remainderHeight, boxWidth, 18);

				blit(stack, x, y + boxHeight - 7, boxWidth, 7, 0, 11, boxWidth, 7, boxWidth, 18);
			}
		} else if (boxHeight < 18) {
			blit(stack, x, y, 7, boxHeight, 0, 0, 7, boxHeight, 18, boxHeight);
			...
			for (int i = 0; i < widthIterations; i++) {
				blit(stack, x + widthOffset, y, 4, boxHeight, 7, 0, 4, boxHeight, 18, boxHeight);
				widthOffset += 4;
			}
			blit(stack, x + widthOffset, y, remainderWidth, boxHeight, 7, 0, remainderWidth, boxHeight, 18, boxHeight);

			blit(stack, x + boxWidth - 7, y, 7, boxHeight, 11, 0, 7, boxHeight, 18, boxHeight);
		} else {
	...
	private static void draw(MatrixStack stack, int x, int y, int widthOffset, int heightOffset, int textXOffset, int textYOffset, int width, int height) {
