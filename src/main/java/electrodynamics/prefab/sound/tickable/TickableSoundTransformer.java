...
public class TickableSoundTransformer extends TickableSoundTile<TileGenericTransformer> {
    ...
    public TickableSoundTransformer(SoundEvent event, SoundSource source, TileGenericTransformer tile, float volume,
	    float pitch, boolean repeat) {
	...
	if (ElectrodynamicsConfig.INSTANCE.TRANSFORMER_SOUND_LOAD_TARGET.get() > 0) {
	    multiplier = tile.lastTransfer.getValue().getWatts()
		    / ElectrodynamicsConfig.INSTANCE.TRANSFORMER_SOUND_LOAD_TARGET.get();
