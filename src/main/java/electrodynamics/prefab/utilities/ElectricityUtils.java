...
public class ElectricityUtils {
	...
	if (transfer.getVoltage() <= 960.0) {
		...
		if (ItemUtils.isIngredientMember(insulatingItems, armor.getItem())) {
			...
			if (armor.getDamageValue() > armor.getMaxDamage()
				|| armor.hurt(integerDamage, entity.level().random, null)) {
			    armor.setCount(0);
			}
		    }
		    return;
	...
	if (tile != null) {
	    if (tile.getCapability(VoltaicCapabilities.CAPABILITY_ELECTRODYNAMIC_BLOCK, dir)
		    .orElse(CapabilityUtils.EMPTY_ELECTRO) != CapabilityUtils.EMPTY_ELECTRO
		    || tile.getCapability(ForgeCapabilities.ENERGY, dir)
			    .orElse(CapabilityUtils.EMPTY_FE) != CapabilityUtils.EMPTY_FE) {
		return true;
	    }
	}
	return false;
