...
public class ScreenGuidebook extends GenericScreen<ContainerGuidebook> {
	...
	private IFormattableTextComponent mergedText = ElectroTextUtils.empty();
	...
	public ScreenGuidebook(ContainerGuidebook screenContainer, PlayerInventory inv, ITextComponent titleIn) {
				...
				for (AbstractWrapperObject<?> data : chapter.pageData) {
					...
					if (data instanceof TextWrapperObject) {
						...
						if (textWrapper == TextWrapperObject.BLANK_LINE) {
							...
							IFormattableTextComponent indentions = ElectroTextUtils.empty();
	...
	private Page writeCurrentTextToPage(Page currentPage, Chapter chapter) {
		...
		if (mergedText.equals(ElectroTextUtils.empty())) {
			return currentPage;
		}
		...
		List<ITextProperties> text = new ArrayList<>(font.getSplitter().splitLines(mergedText, TEXT_WIDTH, Style.EMPTY));
		mergedText = ElectroTextUtils.empty();
	...
	@Override
	protected void renderBg(MatrixStack stack, float partialTick, int x, int y) {
	...
	private void renderPageBackground(MatrixStack stack, int xShift, int guiWidth, int guiHeight, Page page) {
	...
	private void renderPageLabels(MatrixStack stack, int xPageShift, int refX, int refY, Page page) {

		for (TextWrapper text : page.text) {

			if (text.centered) {
				int xShift = (TEXT_WIDTH - font.width(text.characters)) / 2;
				font.draw(stack, LanguageMap.getInstance().getVisualOrder(text.characters), text.x + refX + xShift + xPageShift, refY + text.y, text.color.color());
			} else {
				font.draw(stack, LanguageMap.getInstance().getVisualOrder(text.characters), text.x + refX + xPageShift, text.y + refY, text.color.color());
			}
	...
	private void renderPageTooltips(MatrixStack stack, int xPageShift, int mouseX, int mouseY, int refX, int refY, int xAxis, int yAxis, int guiWidth, int guiHeight, Page page) {
	...
	protected void goToChapterPage() {
		...
		if (page instanceof ModulePage) {
			ModulePage module = (ModulePage) page;
			setPageNumber(module.getPage());
		} else if (page instanceof ChapterPage) {
			ChapterPage chapter = (ChapterPage) page;
	...
	private void movePage(int number) {
		...
		currPageNumber = MathHelper.clamp(currPageNumber, 0, pages.size() - 4);
	}
	...
	private CoverPage getCoverPage() {
		...
		List<ITextProperties> split = font.getSplitter().splitLines(ElectroTextUtils.guidebook("title").withStyle(TextFormatting.BOLD), TEXT_WIDTH, Style.EMPTY);

		int y = 16;

		for (ITextProperties text : split) {
	...
	private CoverPage getSearchPageLeft() {
		...
		List<ITextProperties> split = font.getSplitter().splitLines(ElectroTextUtils.guidebook("searchparameters").withStyle(TextFormatting.BOLD), TEXT_WIDTH, Style.EMPTY);
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
		...
		if (currPageNumber >= pages.size() - 2) {
			...
			scrollIndex = MathHelper.clamp(scrollIndex + scrollDelta, minScroll, maxScroll);
	...
	private void resetSearchButtons() {
		for (ButtonSearchedText button : searchButtons) {
			button.setLine(ElectroTextUtils.empty());
			button.setChapter(ElectroTextUtils.empty());
	...
	private static class SearchHit {
		...
		public final ITextProperties text;
		public final int page;
		public final Chapter chapter;

		public SearchHit(ITextProperties text, int page, Chapter chapter) {
			this.text = text;
			this.page = page;
