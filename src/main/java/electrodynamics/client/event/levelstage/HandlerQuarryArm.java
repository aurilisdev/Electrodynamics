...
public class HandlerQuarryArm extends AbstractLevelStageHandler {
	...
	TextureAtlasSprite armTexture = ElectrodynamicsClientRegister
		.getSprite(ElectrodynamicsClientRegister.TEXTURE_QUARRYARM);
	...
	TextureAtlasSprite darkArmTexture = ElectrodynamicsClientRegister
		.getSprite(ElectrodynamicsClientRegister.TEXTURE_QUARRYARM_DARK);
	...
	TextureAtlasSprite titaniumTexture = minecraft.getTextureAtlas(InventoryMenu.BLOCK_ATLAS)
		.apply(SubtypeDrillHead.titanium.blockTextureLoc);
	...
	armsToRender.forEach((pos, data) -> {
	    BakedModel wheelStill = Minecraft.getInstance().getModelManager()
		    .getModel(ElectrodynamicsClientRegister.MODEL_QUARRYWHEEL_STILL);
	    BakedModel wheelRot = Minecraft.getInstance().getModelManager()
		    .getModel(ElectrodynamicsClientRegister.MODEL_QUARRYWHEEL_ROT);
	    ...
	    stack.mulPose(
		    MathUtils.rotQuaternionDeg(data.leftWheel().xAxisRotation(), 0, data.leftWheel().zAxisRotation()));
	    // stack.mulPose(new Quaternion(data.leftWheel().xAxisRotation(), 0,
	    ...
	    stack.pushPose();
	    stack.translate(0.0, 0.0625, 0.0);
	    stack.mulPose(MathUtils.rotQuaternionDeg(data.rightWheel().xAxisRotation(), 0,
		    data.rightWheel().zAxisRotation()));
	    // stack.mulPose(new Quaternion(data.rightWheel().xAxisRotation(), 0,
	    ...
	    stack.pushPose();
	    stack.translate(0.0, 0.0625, 0.0);
	    stack.mulPose(MathUtils.rotQuaternionDeg(data.bottomWheel().xAxisRotation(), 0,
		    data.bottomWheel().zAxisRotation()));
	    // stack.mulPose(new Quaternion(data.bottomWheel().xAxisRotation(), 0,
	    ...
	    stack.translate(0.0, 0.0625, 0.0);
	    stack.mulPose(
		    MathUtils.rotQuaternionDeg(data.topWheel().xAxisRotation(), 0, data.topWheel().zAxisRotation()));
	    // stack.mulPose(new Quaternion(data.topWheel().xAxisRotation(), 0,
	...
	armsToRender.forEach((pos, data) -> {
	    ...
	    AABB beam = new AABB(0.4375, 0.5625, 0.4375, deltaX * data.signs()[0] + 0.5625, 0.6875,
		    deltaZ * data.signs()[0] + 0.5625);

	    if (frustum.isVisible(beam.move(start))) {
		stack.pushPose();
		stack.translate(start.getX(), start.getY(), start.getZ());
		RenderingUtils.renderFilledBoxNoOverlay(stack, lineBuilder, beam, 1.0F, 0, 0, alpha, u0White, v0White,
			u1White, v1White, 255, RenderingUtils.ALL_FACES);
		stack.popPose();
	    }

	    deltaX = farCorner.getX() - start.getX();
	    deltaZ = farCorner.getZ() - start.getZ();
	    beam = new AABB(0.4375, 0.5625, 0.4375, deltaX * data.signs()[1] + 0.5625, 0.6875,
		    deltaZ * data.signs()[1] + 0.5625);
	    ...
	    beam = new AABB(0.4375, 0.5625, 0.4375, deltaX * data.signs()[2] + 0.5625, 0.6875,
		    deltaZ * data.signs()[2] + 0.5625);
	    ...
	    beam = new AABB(0.4375, 0.5625, 0.4375, deltaX * data.signs()[3] + 0.5625, 0.6875,
		    deltaZ * data.signs()[3] + 0.5625);
