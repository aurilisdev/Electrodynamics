...
public class HandlerRailgunTemperature extends AbstractPostGuiOverlayHandler {
	...
	private void renderHeatToolTip(MatrixStack stack, Minecraft minecraft, ItemStack item) {
		...
		ITextComponent currTempText = ElectroTextUtils.tooltip("railguntemp", ChatFormatter.getChatDisplayShort(temperature, DisplayUnits.TEMPERATURE_CELCIUS)).withStyle(TextFormatting.YELLOW);
		ITextComponent maxTempText = ElectroTextUtils.tooltip("railgunmaxtemp", ChatFormatter.getChatDisplayShort(railgun.getMaxTemp(), DisplayUnits.TEMPERATURE_CELCIUS)).withStyle(TextFormatting.YELLOW);
