...
public final class CapeRenderer {
	...
	public static void onPlayerRender(RenderPlayerEvent.Pre event) {
		...
		if (!player.isInvisible() && !player.isFallFlying() && !player.isSleeping()) {
			...
			if (RenderCape.isPresent(player)) {
				cape.render(player, player.getX() - TileEntityRendererDispatcher.instance.camera.getLookVector().x(), player.getY() - TileEntityRendererDispatcher.instance.camera.getLookVector().y(), player.getZ() - TileEntityRendererDispatcher.instance.camera.getLookVector().z(), delta, event.getMatrixStack());
			}
		}
	}
	...
	private static final class RenderCape {
		...
		private void render(PlayerEntity player, double x, double y, double z, float delta, MatrixStack stack) {
			...
			stack.translate(-player.getEyePosition(delta).x, -player.getEyePosition(delta).y + player.getEyeHeight() + (player.isCrouching() ? 1.35 : 1.45), -player.getEyePosition(delta).z);
			...
			if (Holder.INSTANCE.mapList.getOrDefault(player.getName().getString().toLowerCase(), "").equalsIgnoreCase("dev")) {
				...
				GlStateManager._translated(player.getDeltaMovement().x * -rand.nextFloat(), player.getDeltaMovement().y * -rand.nextFloat(), player.getDeltaMovement().z * -rand.nextFloat());
				...
				try {
					...
					for (int i1 = 0; i1 < (int) scale; i1++) {
						...
						bufferBuilder.vertex(player.getDeltaMovement().x() * -scale * 2 + rand.nextFloat() * 30 - 15 + rand.nextFloat() * scale - scale * 0.5, rand.nextFloat() * 30 - 15 + rand.nextFloat() * 20 * 2 - 20, player.getDeltaMovement().z * -scale * 2 + rand.nextFloat() * 30 - 15 + rand.nextFloat() * scale - scale * 0.5)
		...
		private static final class PlayerCollisionResolver implements PlayerResolver {
			...
			public void resolve(PlayerEntity player, Point point) {
				...
				if (player.isCrouching()) {
					...
					float rx = (point.posX - (float) player.getX()) * MathHelper.cos(yaw + (float) Math.PI / 2) + (point.posZ - (float) player.getZ()) * MathHelper.sin(yaw + (float) Math.PI / 2);
