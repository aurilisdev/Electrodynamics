...
public class RenderCompositeArmor extends BipedModel<LivingEntity> {
	...
	@Override
	public void renderToBuffer(MatrixStack matrixStack, IVertexBuilder buffer, int packedLight, int packedOverlay, float pRed, float pGreen, float pBlue, float pAlpha) {
		if (HEAD.visible) {
			HEAD.copyFrom(head.createShallowCopy());
			HEAD.render(matrixStack, Minecraft.getInstance().renderBuffers().bufferSource().getBuffer(RenderType.entityTranslucent(new ResourceLocation(ItemCompositeArmor.ARMOR_TEXTURE_LOCATION))), packedLight, packedOverlay);
		}
		if (CHEST.visible) {
