...
public abstract class AbstractTileRenderer<T extends GenericTile> implements BlockEntityRenderer<T> {
	...
	public void renderItem(ItemStack stack, TransformType type, int light, int overlay, PoseStack poseStack, MultiBufferSource bufferSource, @Nullable Level world, int seed) {
		Minecraft.getInstance().getItemRenderer().renderStatic(stack, type, light, overlay, poseStack, bufferSource, seed);
	}
