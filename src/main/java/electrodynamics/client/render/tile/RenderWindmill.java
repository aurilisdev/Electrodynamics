...
public class RenderWindmill extends AbstractTileRenderer<TileWindmill> {
    ...
    @Override
    public void render(@NotNull TileWindmill tileEntityIn, float partialTicks, @NotNull PoseStack matrixStackIn,
	...
	float partial = (float) (partialTicks * tileEntityIn.rotationSpeed
		* (tileEntityIn.directionFlag.getValue() ? 1 : -1));

	matrixStackIn.mulPose(MathUtils.rotQuaternionDeg(
		(float) ((tileEntityIn.savedTickRotation + partial) * tileEntityIn.generating.getValue() * 1.5), 0, 0));
	// matrixStackIn.mulPose(new Quaternion((float) ((tileEntityIn.savedTickRotation
	// + partial) * tileEntityIn.generating.get() * 1.5), 0, 0, true));
