...
public class RenderChargerGeneric extends AbstractTileRenderer<GenericTileCharger> {
	...
	if (chargingItem != null && !chargingItem.isEmpty() && chargingItem.getItem() instanceof IItemElectric) {
		...
		case NORTH -> {
		    ...
		    // matrixStackIn.mulPose(new Quaternion(new Vector3f(0.0F, 1.0F, 0.0F), 180,
		    // true));
		}
		case EAST -> {
		    ...
		    // matrixStackIn.mulPose(new Quaternion(new Vector3f(0.0F, 1.0F, 0.0F), 90,
		    // true));
		}
		case SOUTH -> {
		...
		case WEST -> {
		    ...
		    // matrixStackIn.mulPose(new Quaternion(new Vector3f(0.0F, 1.0F, 0.0F), 270,
		    // true));
		}
		default -> {
		}
	    ...
	    renderItem(chargingItem, ItemDisplayContext.NONE, combinedLightIn, combinedOverlayIn, matrixStackIn,
		    bufferIn, tileEntityIn.getLevel(), 0);
