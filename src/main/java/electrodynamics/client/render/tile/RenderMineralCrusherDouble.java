...
public class RenderMineralCrusherDouble extends TileEntityRenderer<TileMineralCrusherDouble> {

    public RenderMineralCrusherDouble(TileEntityRendererDispatcher rendererDispatcherIn) {
	super(rendererDispatcherIn);
    }

    @Override
    @Deprecated
    public void render(TileMineralCrusherDouble tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn,
	    int combinedLightIn, int combinedOverlayIn) {
	matrixStackIn.push();
	IBakedModel ibakedmodel = Minecraft.getInstance().getModelManager().getModel(ClientRegister.MODEL_MINERALCRUSHERDOUBLEBASE);
	...
	if (!stack.isEmpty()) {
	    ...
	    matrixStackIn.translate(0.5 + dir.getXOffset() / scale - (dir.getZOffset() != 0 ? 0.07 : 0),
		    stack.getItem() instanceof BlockItem ? 0.48 : 0.39, 0.5 + dir.getZOffset() / scale - (dir.getXOffset() != 0 ? 0.07 : 0));
