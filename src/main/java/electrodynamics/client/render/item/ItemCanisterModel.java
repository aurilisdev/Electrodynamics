...
public class ItemCanisterModel implements IModelGeometry<ItemCanisterModel> {
    ...
    public BakedModel bake(IModelConfiguration owner, ModelBakery bakery, Function<Material, TextureAtlasSprite> sprite, ModelState modelTransform, ItemOverrides overrides, ResourceLocation modelloc) {
		...
		TextureAtlasSprite fluidSprite = fluid != Fluids.EMPTY
			? sprite.apply(ForgeHooksClient.getBlockMaterial(fluid.getAttributes().getStillTexture()))
			: null;
		...
		if (baseLocation != null) {
		    builder.addQuads(ItemLayerModel.getLayerRenderType(false),
			    ItemLayerModel.getQuadsForSprites(ImmutableList.of(baseLocation), transform, sprite));
		}
        ...
        public BakedModel resolve(BakedModel originalModel, ItemStack stack, @Nullable ClientLevel world, @Nullable LivingEntity entity, int seed)
        {
            ...
            return FluidUtil.getFluidContained(stack)
                    .map(fluidStack -> {
                        ...
                        {
                            ItemCanisterModel unbaked = this.parent.withFluid(fluid);
