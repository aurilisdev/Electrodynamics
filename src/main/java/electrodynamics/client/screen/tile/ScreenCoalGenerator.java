...
public class ScreenCoalGenerator extends GenericScreen<ContainerCoalGenerator> {
	...
	addComponent(new ScreenComponentMultiLabel(0, 0, graphics -> {
	    ...
	    TransferPack output = TransferPack.ampsVoltage(
		    ElectroConstants.COALGENERATOR_AMPERAGE
	    ...
	    graphics.drawString(font,
		    ElectroTextUtils.gui("machine.current",
			    ChatFormatter.getChatDisplayShort(output.getAmps(), DisplayUnits.AMPERE)),
		    inventoryLabelX + 60, inventoryLabelY - 40, 4210752, false);
	    graphics.drawString(font,
		    ElectroTextUtils.gui("machine.output",
			    ChatFormatter.getChatDisplayShort(output.getWatts(), DisplayUnits.WATT)),
		    inventoryLabelX + 60, inventoryLabelY - 27, 4210752, false);
	    graphics.drawString(font,
