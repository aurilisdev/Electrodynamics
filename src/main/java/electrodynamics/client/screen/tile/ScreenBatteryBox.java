...
public class ScreenBatteryBox extends GenericScreen<ContainerBatteryBox> {
	...
	addComponent(new ScreenComponentMultiLabel(0, 0, graphics -> {
	    ...
	    graphics.drawString(font, ElectroTextUtils.gui("machine.current",
		    ChatFormatter.getChatDisplayShort(box.powerOutput.getValue() * 20.0
			    * box.currentCapacityMultiplier.getValue() / electro.getVoltage(), DisplayUnits.AMPERE)),
		    inventoryLabelX, inventoryLabelY - 55, 4210752, false);
	    graphics.drawString(font,
		    ElectroTextUtils
			    .gui("machine.transfer",
				    ChatFormatter.getChatDisplayShort(box.powerOutput.getValue() * 20.0
					    * box.currentCapacityMultiplier.getValue(), DisplayUnits.WATT)),
	    ...
	    graphics.drawString(font, ElectroTextUtils.gui("machine.stored", VoltaicTextUtils.ratio(
		    ChatFormatter.getChatDisplayShort(electro.getJoulesStored(), DisplayUnits.JOULES),
		    ChatFormatter.getChatDisplayShort(
			    box.maxJoules.getValue() * box.currentCapacityMultiplier.getValue(), DisplayUnits.JOULES))),
	...
	list.add(
		ElectroTextUtils
			.gui("machine.current",
				ChatFormatter
					.getChatDisplayShort(
						box.powerOutput.getValue() * 20.0
							* box.currentCapacityMultiplier.getValue() / el.getVoltage(),
						DisplayUnits.AMPERE)
	...
	list.add(
		ElectroTextUtils
			.gui("machine.transfer",
				ChatFormatter
					.getChatDisplayShort(box.powerOutput.getValue() * 20.0
						* box.currentCapacityMultiplier.getValue(), DisplayUnits.WATT)
					.withStyle(ChatFormatting.GRAY))
			.withStyle(ChatFormatting.DARK_GRAY).getVisualOrderText());
	list.add(ElectroTextUtils
		.gui("machine.transfer",
			VoltaicTextUtils.ratio(
				ChatFormatter.getChatDisplayShort(
					box.powerOutput.getValue() * box.currentCapacityMultiplier.getValue(),
					DisplayUnits.FORGE_ENERGY_UNIT).withStyle(ChatFormatting.GRAY),
		...
		ElectroTextUtils
			.gui("machine.stored",
				VoltaicTextUtils.ratio(
					...
					ChatFormatter.getChatDisplayShort(
						box.maxJoules.getValue() * box.currentCapacityMultiplier.getValue(),
						DisplayUnits.JOULES))
					.withStyle(ChatFormatting.GRAY))
			.withStyle(ChatFormatting.DARK_GRAY).getVisualOrderText());
