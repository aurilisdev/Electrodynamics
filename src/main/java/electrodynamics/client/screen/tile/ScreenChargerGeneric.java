...
public class ScreenChargerGeneric extends GenericScreen<ContainerChargerGeneric> {
	...
	public ScreenChargerGeneric(ContainerChargerGeneric screenContainer, Inventory inv, Component titleIn) {
		...
		addComponent(new ScreenComponentMultiLabel(0, 0, poseStack -> {
			...
			font.draw(poseStack, ElectroTextUtils.gui("genericcharger.chargeperc", ChatFormatter.getChatDisplayShort(chargingPercentage, DisplayUnits.PERCENTAGE)).withStyle(ChatFormatting.DARK_GRAY).withStyle(ChatFormatting.DARK_GRAY), inventoryLabelX, 33, Color.BLACK.color());

			Component capable = TextComponent.EMPTY;
