...
public class ScreenSeismicScanner extends GenericScreen<ContainerSeismicScanner> {
	...
	addComponent(button = (ScreenComponentButton<?>) new ScreenComponentButton<>(
		ScreenComponentGuiTab.GuiInfoTabTextures.REGULAR, -AbstractScreenComponentInfo.SIZE + 1,
		AbstractScreenComponentInfo.SIZE + 2).onTooltip((graphics, but, xAxis, yAxis) -> {
	...
	componentsToShow.add(addComponent(new ScreenComponentSimpleLabel(15, 23, 10, Color.TEXT_GRAY,
		ElectroTextUtils.gui("seismicscanner.material"))));
	componentsToShow.add(addComponent(new ScreenComponentSimpleLabel(15, 63, 10, Color.TEXT_GRAY,
		ElectroTextUtils.gui("seismicscanner.pattern"))));
	componentsToShow.add(addComponent(new ScreenComponentSimpleLabel(20, 89, 10, Color.TEXT_GRAY,
		() -> getPatternName(menu.getOwnerItem()))));
	componentsToShow
		.add(addComponent(new ScreenComponentSimpleLabel(20, 99, 10, Color.TEXT_GRAY, () -> ElectroTextUtils
			.gui("seismicscanner.patternintegrity", getPatternDurability(menu.getOwnerItem())))));

	componentsToHide.add(addComponent(new ScreenComponentSimpleLabel(20, 80, 10, Color.TEXT_GRAY,
	...
	componentsToHide.add(addComponent(new ScreenComponentMultiLabel(0, 0, graphics -> {
		...
		graphics.drawString(font, ElectroTextUtils.gui("seismicscanner.xcoordna"), 30, 90,
			Color.TEXT_GRAY.color(), false);
		graphics.drawString(font, ElectroTextUtils.gui("seismicscanner.ycoordna"), 30, 100,
			Color.TEXT_GRAY.color(), false);
		graphics.drawString(font, ElectroTextUtils.gui("seismicscanner.zcoordna"), 30, 110,
			Color.TEXT_GRAY.color(), false);
	    } else {
		graphics.drawString(font, ElectroTextUtils.gui("seismicscanner.xcoord", blockLoc.intX()), 30, 90,
			Color.TEXT_GRAY.color(), false);
		graphics.drawString(font, ElectroTextUtils.gui("seismicscanner.ycoord", blockLoc.intY()), 30, 100,
			Color.TEXT_GRAY.color(), false);
		graphics.drawString(font, ElectroTextUtils.gui("seismicscanner.zcoord", blockLoc.intZ()), 30, 110,
			Color.TEXT_GRAY.color(), false);
	    }
	})));
	...
	componentsToHide.add(addComponent(new ScreenComponentButton<>(15, 46, 120, 20)
		...
		.setOnPress(button -> {
		    ...
		    PacketDistributor.sendToServer(new PacketSeismicScanner(inv.player.getUUID(),
			    PacketSeismicScanner.Type.switchsonarmode, mode.ordinal(), hand.ordinal()));
		}).setLabel(() -> {
		    //
		    ItemStack owner = menu.getOwnerItem();
		    if (owner.isEmpty()) {
		    ...
		    return mode == ItemSeismicScanner.ScannerMode.PASSIVE
			    ? ElectroTextUtils.gui("seismicscanner.scanpassive")
	...
	if (pattern == Blocks.AIR) {
	    ...
	    graphics.blit(icon.getLocation(), x, y, icon.textureU(), icon.textureV(), icon.textureWidth(),
		    icon.textureHeight(), icon.imageWidth(), icon.imageHeight());
	} else {
