...
public class ElectricityDisplay {
	...
	public static String getDisplay(double value, Unit unit, int decimalPlaces, boolean isShort)
	{
		...
		if (checkValue < 1.0D)
		{
			return roundPreciseStatic(Measurement.MILLI.process(value), decimalPlaces) + " " + Measurement.MILLI.getName(isShort) + unitName;
		} else if (checkValue > Measurement.GIGA.value)
		{
			return roundPreciseStatic(Measurement.GIGA.process(value), decimalPlaces) + " " + Measurement.GIGA.getName(isShort) + unitName;
		} else if (checkValue > Measurement.MEGA.value)
		{
			return roundPreciseStatic(Measurement.MEGA.process(value), decimalPlaces) + " " + Measurement.MEGA.getName(isShort) + unitName;
		} else if (checkValue > Measurement.KILO.value)
		{
			return roundPreciseStatic(Measurement.KILO.process(value), decimalPlaces) + " " + Measurement.KILO.getName(isShort) + unitName;
		}
