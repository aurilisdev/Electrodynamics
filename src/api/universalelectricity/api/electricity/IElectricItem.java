...
public interface IElectricItem {
	...
	{
		if (container.getTagCompound() == null || !container.getTagCompound().hasKey(UniversalElectricity.ELECTRICITY_STORED_TAG))
		{
			return 0;
		}
	...
	default int receiveElectricity(ItemStack container, int maxReceive, boolean simulate)
	{
		...
		int energy = container.getTagCompound().getInteger(UniversalElectricity.ELECTRICITY_STORED_TAG);
		int energyReceived = Math.min(getElectricCapacity(container) - energy, Math.min(getReceiveRate(container), maxReceive));
