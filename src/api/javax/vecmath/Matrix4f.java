...
public class Matrix4f implements java.io.Serializable, Cloneable {
	...
	public float				m33;
	/*
	 * double[] tmp = new double[9]; double[] tmp_scale = new double[3]; double[]
	 * tmp_rot = new double[9];
	 */
	...
	/**
	 * Constructs and initializes a Matrix4f from the quaternion, translation, and
	 * scale values; the scale is applied only to the rotational components of the
	 * matrix (upper 3x3) and not to the translational components.
	 *
	 * @param q1
	...
	/**
	 * Constructs and initializes a Matrix4f from the rotation matrix, translation,
	 * and scale values; the scale is applied only to the rotational components of
	 * the matrix (upper 3x3) and not to the translational components of the matrix.
	 *
	 * @param m1
	 ...
	 * scale (by doing an SVD) from the rotational component and multiplying by the
	 * new scale.
	 *
	 * @param scale
	...
	/**
	 * Performs an SVD normalization of this matrix to calculate the rotation as a
	 * 3x3 matrix, the translation, and the scale. None of the matrix values are
	 * modified.
	 *
	 * @param m1
	...
	/**
	 * Sets the specified row of this matrix4f to the four values provided in the
	 * passed array.
	 *
	 * @param row
	 *            the row number to be modified (zero indexed)
	...
	/**
	 * Performs an element-by-element subtraction of matrix m2 from matrix m1 and
	 * places the result into matrix this (this = m2 - m1).
	 *
	 * @param m1
	 *            the first matrix
	 * @param m2
	...
	{
		m00 = 1.0f - 2.0f * q1.y * q1.y - 2.0f * q1.z * q1.z;
		m10 = 2.0f * (q1.x * q1.y + q1.w * q1.z);
		m20 = 2.0f * (q1.x * q1.z - q1.w * q1.y);

		m01 = 2.0f * (q1.x * q1.y - q1.w * q1.z);
		m11 = 1.0f - 2.0f * q1.x * q1.x - 2.0f * q1.z * q1.z;
		m21 = 2.0f * (q1.y * q1.z + q1.w * q1.x);

		m02 = 2.0f * (q1.x * q1.z + q1.w * q1.y);
		m12 = 2.0f * (q1.y * q1.z - q1.w * q1.x);
		m22 = 1.0f - 2.0f * q1.x * q1.x - 2.0f * q1.y * q1.y;
	...
	{
		m00 = (float) (1.0 - 2.0 * q1.y * q1.y - 2.0 * q1.z * q1.z);
		m10 = (float) (2.0 * (q1.x * q1.y + q1.w * q1.z));
		m20 = (float) (2.0 * (q1.x * q1.z - q1.w * q1.y));

		m01 = (float) (2.0 * (q1.x * q1.y - q1.w * q1.z));
		m11 = (float) (1.0 - 2.0 * q1.x * q1.x - 2.0 * q1.z * q1.z);
		m21 = (float) (2.0 * (q1.y * q1.z + q1.w * q1.x));

		m02 = (float) (2.0 * (q1.x * q1.z + q1.w * q1.y));
		m12 = (float) (2.0 * (q1.y * q1.z - q1.w * q1.x));
		m22 = (float) (1.0 - 2.0 * q1.x * q1.x - 2.0 * q1.y * q1.y);
	...
	{
		m00 = s * (1.0f - 2.0f * q1.y * q1.y - 2.0f * q1.z * q1.z);
		m10 = s * (2.0f * (q1.x * q1.y + q1.w * q1.z));
		m20 = s * (2.0f * (q1.x * q1.z - q1.w * q1.y));

		m01 = s * (2.0f * (q1.x * q1.y - q1.w * q1.z));
		m11 = s * (1.0f - 2.0f * q1.x * q1.x - 2.0f * q1.z * q1.z);
		m21 = s * (2.0f * (q1.y * q1.z + q1.w * q1.x));

		m02 = s * (2.0f * (q1.x * q1.z + q1.w * q1.y));
		m12 = s * (2.0f * (q1.y * q1.z - q1.w * q1.x));
		m22 = s * (1.0f - 2.0f * q1.x * q1.x - 2.0f * q1.y * q1.y);
	...
	/**
	 * Sets the rotational component (upper 3x3) of this matrix to the matrix values
	 * in the single precision Matrix3f argument; the other elements of this matrix
	 * are initialized as if this were an identity matrix (i.e., affine matrix with
	...
	/**
	 * Sets the rotational component (upper 3x3) of this matrix to the matrix values
	 * in the double precision Matrix3d argument; the other elements of this matrix
	 * are initialized as if this were an identity matrix (i.e., affine matrix with
	 * no translational component).
	 *
	 * @param m1
	...
	/**
	 * Sets the values in this Matrix4f equal to the row-major array parameter (ie,
	 * the first four elements of the array will be copied into the first row of
	 * this matrix, etc.).
	 *
	 * @param m
	...
	/**
	 * Sets the value of this transform to a scale and translation matrix; the scale
	 * is not applied to the translation and all of the matrix values are modified.
	 *
	 * @param scale
	 *            the scale factor for the matrix
	 * @param t1
	 ...
	 * translation is scaled by the scale factor and all of the matrix values are
	 * modified.
	 *
	 * @param t1
	 *            the translation amount
	 ...
	 * matrix m1, the translation t1, and the scale factor. The translation is not
	...
	{
		m00 = (float) (m1.m00 * scale);
		m01 = (float) (m1.m01 * scale);
		m02 = (float) (m1.m02 * scale);
		m03 = (float) t1.x;

		m10 = (float) (m1.m10 * scale);
		m11 = (float) (m1.m11 * scale);
		m12 = (float) (m1.m12 * scale);
		m13 = (float) t1.y;

		m20 = (float) (m1.m20 * scale);
		m21 = (float) (m1.m21 * scale);
		m22 = (float) (m1.m22 * scale);
		m23 = (float) t1.z;
	...
	public boolean equals(Matrix4f m1)
	{
		try
		{
			return m00 == m1.m00 && m01 == m1.m01 && m02 == m1.m02 && m03 == m1.m03 && m10 == m1.m10 && m11 == m1.m11 && m12 == m1.m12 && m13 == m1.m13 && m20 == m1.m20 && m21 == m1.m21 && m22 == m1.m22 && m23 == m1.m23 && m30 == m1.m30
					&& m31 == m1.m31 && m32 == m1.m32 && m33 == m1.m33;
	...
	/**
	 * Returns true if the Object t1 is of type Matrix4f and all of the data members
	 * of t1 are equal to the corresponding data members in this Matrix4f.
	 *
	 * @param t1
	 *            the matrix with which the comparison is made.
	 * @return true or false
	...
	public boolean equals(Object t1)
	{
		try
		{
			...
			return m00 == m2.m00 && m01 == m2.m01 && m02 == m2.m02 && m03 == m2.m03 && m10 == m2.m10 && m11 == m2.m11 && m12 == m2.m12 && m13 == m2.m13 && m20 == m2.m20 && m21 == m2.m21 && m22 == m2.m22 && m23 == m2.m23 && m30 == m2.m30
	 ...
	 * L-infinite distance is equal to MAX[i=0,1,2,3 ; j=0,1,2,3 ; abs(this.m(i,j) -
	 * m1.m(i,j)]
	 *
	 * @param m1
	 ...
	 * different Matrix4f objects with identical data values (i.e., Matrix4f.equals
	...
	/**
	 * Transform the vector vec using this Transform and place the result back into
	 * vec.
	 *
	 * @param vec
	 *            the single precision vector to be transformed
	 ...
	 * pointOut. The fourth element of the point input paramter is assumed to be
	 * one.
	 *
	 * @param point
	...
	/**
	 * Transforms the point parameter with this Matrix4f and places the result back
	 * into point. The fourth element of the point input paramter is assumed to be
	 * one.
	 *
	 * @param point
	 *            the input point to be transformed.
	 ...
	 * Sets the rotational component (upper 3x3) of this matrix to the matrix values
	 * in the double precision Matrix3d argument; the other elements of this matrix
	 * are unchanged; a singular value decomposition is performed on this object's
	 * upper 3x3 matrix to factor out the scale, then this object's upper 3x3 matrix
	 * components are replaced by the passed rotation components, and then the scale
	 ...
	 * object's upper 3x3 matrix to factor out the scale, then this object's upper
	 * 3x3 matrix components are replaced by the matrix equivalent of the
