...
public class ContainerBase<T extends IPlayerUsing> extends Container {
		...
		if (slot >= 0 && slot < inventorySlots.size()) {
			...
			if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
				...
				for (Object crafter : crafters) {
					((ICrafting) crafter).sendSlotContents(this, slot, itemstack1);
				}
			}
		}
		return toReturn;
	...
	protected boolean mergeItemStack(ItemStack stack, int min, int max, boolean negative) {
		...
		if (stack.isStackable()) {
			while (stack.stackSize > 0 && ((negative ? k >= min : k < max))) {
	...
	public boolean canInteractWith(EntityPlayer entityplayer) {
		return host instanceof IInventory ? ((IInventory) host).isUseableByPlayer(entityplayer) : (boolean) (host instanceof TileEntity ? entityplayer.getDistanceSq(((TileEntity) host).xCoord, ((TileEntity) host).yCoord, ((TileEntity) host).zCoord) <= 64 : true);
	}
}
