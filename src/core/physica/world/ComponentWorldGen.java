...
public class ComponentWorldGen implements IWorldGenerator {
	...
	private void generateOverworld(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider)
	{
		...
		{
			generateOre(ComponentBlocks.blockOre.getStateFromMeta(ore.ordinal()), world, random, chunkX * 16, chunkZ * 16, ore.getMinY(), ore.getMaxY(), (int) (ore.getVeinSize() / 2.0f + random.nextFloat() * ore.getVeinSize() / 2.0f),
					ore.getVeinsPerChunk());
		}
	}

	private void generateOre(IBlockState ore, World world, Random random, int x, int z, int minY, int maxY, int veinSize, int veinsPerChunk)
	{
