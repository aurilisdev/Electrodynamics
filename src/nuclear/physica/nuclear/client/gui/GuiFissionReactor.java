...
public class GuiFissionReactor extends GuiContainerBase<TileFissionReactor> implements IBaseUtilities {
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		double ticksLeft = 0;
		if (host.hasFuelRod()) {
			ticksLeft = (host.getStackInSlot(TileFissionReactor.SLOT_INPUT).getMaxDamage() - host.getStackInSlot(TileFissionReactor.SLOT_INPUT).getItemDamage()) / (+1 + Math.round(host.getTemperature() / (TileFissionReactor.MELTDOWN_TEMPERATURE / 2.0f)));
		}
