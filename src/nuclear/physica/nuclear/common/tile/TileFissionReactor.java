...
public class TileFissionReactor extends TileBaseContainer implements IGuiInterface {
	...
	public void updateCommon(int ticks) {
		...
		if (isServer() ? ticks % 5 == 0 : true) {
			...
			List<EntityLiving> entities = World().getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getBoundingBox(loc.xCoord - dRadius, loc.yCoord - dRadius, loc.zCoord - dRadius, loc.xCoord + dRadius + 1, loc.yCoord + tempScale, loc.zCoord + dRadius + 1));
			for (EntityLiving entity : entities) {
				if (entity != null) {
					double scale = (tempScale - entity.getDistance(loc.xCoord + 0.5, loc.yCoord + 0.5, loc.zCoord + 0.5)) / 3.0;
					RadiationSystem.applyRontgenEntity(entity, (float) scale * 1.5f, (float) scale * 15, (float) entity.getDistance(loc.xCoord + 0.5, loc.yCoord + 0.5, loc.zCoord + 0.5), (float) tempScale);
				}
			}
		}
		...
		for (int k = 0; k < 4; k++) {
			...
			for (float i = 0.175f; i < 0.8; i += 0.1) {
				if (World().rand.nextFloat() < (temperature - AIR_TEMPERATURE) / (MELTDOWN_TEMPERATURE * 3) / 4.0) {
					World().spawnParticle("reddust", loc.xCoord + xCoordOffset + 0.5f + World().rand.nextDouble() * radius - radius / 2, loc.yCoord + i + World().rand.nextDouble() * radius - radius / 2, loc.zCoord + zCoordOffset + 0.5f + World().rand.nextDouble() * radius - radius / 2, 0.01f, 0.4f, 0.01f);
				}
			}
		}
			...
			for (int i = 0; i < (int) (Math.pow(radius, 1.5) / 10f); i++) {
				for (int k = 0; k < 4; k++) {
					if (World().rand.nextFloat() < 1.0 / 16.0) {
						...
						float xCoordOffset = k == 0 ? -outerRods : k == 1 ? outerRods : 0;
						float zCoordOffset = k == 2 ? -outerRods : k == 3 ? outerRods : 0;
						World().spawnParticle("reddust", loc.xCoord + xCoordOffset + 0.5f + World().rand.nextDouble() * radius - radius / 2, loc.yCoord + World().rand.nextDouble() * radius - radius / 2, loc.zCoord + zCoordOffset + 0.5f + World().rand.nextDouble() * radius - radius / 2, 0.01f, 0.5f, 0.01f);
					}
				}
			}
		}
	...
	private void processFuelRod() {
		...
		if (isHighEnriched()) {
			if (fuelRod.getItemDamage() >= fuelRod.getMaxDamage()) {
				setInventorySlotContents(SLOT_INPUT, new ItemStack(NuclearItemRegister.itemLowEnrichedFuelCell, 1, (int) (NuclearItemRegister.itemLowEnrichedFuelCell.getMaxDamage() / 3 + World().rand.nextFloat() * (NuclearItemRegister.itemLowEnrichedFuelCell.getMaxDamage() / 5))));
			}
