...
public class TileQuantumAssembler extends TileBasePoweredContainer implements IGuiInterface {
	...
	public boolean canProcess() {
		...
		for (int i = 0; i <= 5; i++) {
			...
			if ((itemStackInSlot == null) || (itemStackInSlot.getItem() != NuclearItemRegister.itemDarkmatterCell)) {
				return false;
			}
		}
	...
	public boolean isRestricted(ItemStack itemStack) {
		return itemStack == null || itemStack.stackSize <= 0 || itemStack.getItem() == NuclearItemRegister.itemDarkmatterCell || (ConfigNuclearPhysics.FLIP_BLACKLIST_TO_WHITELIST ? !ConfigNuclearPhysics.QUANTUM_ASSEMBLER_BLACKLIST.contains(itemStack.getUnlocalizedName()) : ConfigNuclearPhysics.QUANTUM_ASSEMBLER_BLACKLIST.contains(itemStack.getUnlocalizedName()));
	}
	...
	public boolean canConnectElectricity(Face from) {
		return Face.DOWN.equals(from) || Face.UP.equals(from);
	}
