...
public class TileChemicalBoiler extends TileBasePoweredContainer implements IGuiInterface, IFluidHandler {
	...
	public boolean canConnectElectricity(Face from) {
		return from == getFacing().getOpposite() || from == Face.DOWN;
	}
	...
	public boolean canFill(ForgeDirection from, Fluid fluid) {
		return fluid == FluidRegistry.WATER ? from.ordinal() == getFacing().getOpposite().getRelativeSide(Face.WEST).ordinal() : fluid == NuclearFluidRegister.LIQUID_HE ? from.ordinal() == getFacing().getOpposite().getRelativeSide(Face.EAST).ordinal() : false;
	}
	...
	public FluidTankInfo[] getTankInfo(ForgeDirection from) {
		return from.ordinal() == getFacing().getOpposite().getRelativeSide(Face.WEST).ordinal() ? new FluidTankInfo[] { waterTank.getInfo() } : from.ordinal() == getFacing().getOpposite().getRelativeSide(Face.EAST).ordinal() ? new FluidTankInfo[] { hexaTank.getInfo() } : new FluidTankInfo[] {};
	}
