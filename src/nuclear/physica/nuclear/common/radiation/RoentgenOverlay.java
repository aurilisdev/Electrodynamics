...
public class RoentgenOverlay implements IBaseUtilities {
	...
	public void onGameOverlay(RenderGameOverlayEvent.Text event) {
		...
		if (use != null && use.getItem() == NuclearItemRegister.itemGeigerCounter) {
			GL11.glPushMatrix();
			double rounded = NuclearItemRegister.itemGeigerCounter.getElectricityStored(use) > 5 ? roundPrecise(cachedRoentgen * RadiationSystem.toRealRoentgenConversionRate, 2) : roundPrecise(Math.min(3.6, cachedRoentgen * RadiationSystem.toRealRoentgenConversionRate), 2);
