...
public class ConstructorCalculationThread extends Thread {
	...
	public void run() {
		...
		if (!constructor.isInvalid()) {
			...
			boolean interiorModule = constructor.getModuleCount(ForcefieldItemRegister.moduleMap.get("moduleUpgradeInterior"), TileFortronFieldConstructor.SLOT_UPGRADES[0], TileFortronFieldConstructor.SLOT_UPGRADES[TileFortronFieldConstructor.SLOT_UPGRADES.length - 1]) > 0;
	...
	private void calculateCube(boolean interior, int x, int y, int z) {
		...
		int xRadiusPos = (int) (x + Math.min(64, constructor.getModuleCountIn(scaleModule, TileFortronFieldConstructor.SLOT_EAST[0], TileFortronFieldConstructor.SLOT_EAST[1])) / (interior ? ConfigForcefields.FORCEFIELD_INTERIOR_MODULE_DOWNSIZE : 1));
		int yRadiusPos = (int) Math.min(255, Math.max(0, y + constructor.getModuleCountIn(scaleModule, TileFortronFieldConstructor.SLOT_UP[0], TileFortronFieldConstructor.SLOT_UP[1]) / (interior ? ConfigForcefields.FORCEFIELD_INTERIOR_MODULE_DOWNSIZE : 1)));
		int zRadiusPos = (int) (z + Math.min(64, constructor.getModuleCountIn(scaleModule, TileFortronFieldConstructor.SLOT_SOUTH[0], TileFortronFieldConstructor.SLOT_SOUTH[1])) / (interior ? ConfigForcefields.FORCEFIELD_INTERIOR_MODULE_DOWNSIZE : 1));
		int xRadiusNeg = (int) (x - Math.min(64, constructor.getModuleCountIn(scaleModule, TileFortronFieldConstructor.SLOT_WEST[0], TileFortronFieldConstructor.SLOT_WEST[1])) / (interior ? ConfigForcefields.FORCEFIELD_INTERIOR_MODULE_DOWNSIZE : 1));
		int yRadiusNeg = (int) Math.max(0, y - constructor.getModuleCountIn(scaleModule, TileFortronFieldConstructor.SLOT_DOWN[0], TileFortronFieldConstructor.SLOT_DOWN[1]) / (interior ? ConfigForcefields.FORCEFIELD_INTERIOR_MODULE_DOWNSIZE : 1));
		int zRadiusNeg = (int) (z - Math.min(64, constructor.getModuleCountIn(scaleModule, TileFortronFieldConstructor.SLOT_NORTH[0], TileFortronFieldConstructor.SLOT_NORTH[1])) / (interior ? ConfigForcefields.FORCEFIELD_INTERIOR_MODULE_DOWNSIZE : 1));
	...
	private void calculatePyramid(boolean interior, int x, int y, int z) {
		...
		double zPos = z + Math.min(64, constructor.getModuleCountIn(scaleModule, TileFortronFieldConstructor.SLOT_SOUTH[0], TileFortronFieldConstructor.SLOT_SOUTH[1])) / (interior ? ConfigForcefields.FORCEFIELD_INTERIOR_MODULE_DOWNSIZE : 1);
		...
		double zNeg = z - Math.min(64, constructor.getModuleCountIn(scaleModule, TileFortronFieldConstructor.SLOT_NORTH[0], TileFortronFieldConstructor.SLOT_NORTH[1])) / (interior ? ConfigForcefields.FORCEFIELD_INTERIOR_MODULE_DOWNSIZE : 1);
