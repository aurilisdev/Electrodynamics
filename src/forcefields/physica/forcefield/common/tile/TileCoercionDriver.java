...
public class TileCoercionDriver extends TileBasePoweredContainer implements IInvFortronTile, IGuiInterface {
	...
	public int getMaxEnergyStored() {
		return (int) (BASE_ENERGY + BASE_ENERGY * 10 * Math.pow(1.051, getModuleCount(ForcefieldItemRegister.moduleMap.get("moduleUpgradeSpeed"), SLOT_MODULE1, SLOT_MODULE3)) + BASE_ENERGY * 30 * Math.pow(1.051, getModuleCount(ForcefieldItemRegister.moduleMap.get("moduleUpgradeCapacity"), SLOT_MODULE1, SLOT_MODULE3) * 2));
	}
	...
	public void updateServer(int ticks) {
		...
		if (getElectricityStored() > 1) {
			setElectricityStored(getElectricityStored() - fortronTank.fill(new FluidStack(ForcefieldFluidRegister.LIQUID_FORTRON, Math.min(getPowerUsage(), getElectricityStored()) - 1), true));
		}
		if (canSendBeam()) {
