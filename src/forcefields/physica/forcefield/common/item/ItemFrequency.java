...
public class ItemFrequency extends ItemDescriptable {
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
		if (!world.isRemote) {
			...
			if (blockHit != ForcefieldBlockRegister.blockCoercionDriver && blockHit != ForcefieldBlockRegister.blockFortronCapacitor && blockHit != ForcefieldBlockRegister.blockFortronConstructor && blockHit != ForcefieldBlockRegister.blockInterdictionMatrix) {
	...
	public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if (stack.hasTagCompound()) {
			if (stack.getTagCompound().hasKey("frequency")) {
				...
				if (tile instanceof IInvFortronTile) {
