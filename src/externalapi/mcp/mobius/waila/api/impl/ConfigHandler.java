...
public class ConfigHandler implements IWailaConfigHandler {
	...
	public HashMap<String, String> getConfigKeys(String modName)
	{
		...
		{
			return modules.get(modName).options;
		} else
		{
			return null;
		}
	...
	public void addConfig(String modName, String key, String name, boolean defvalue)
	{
		...
		if (!modules.containsKey(modName))
		{
			modules.put(modName, new ConfigModule(modName));
		}

		modules.get(modName).addOption(key, name);
	}
	...
	public boolean getConfig(String key, boolean defvalue)
	{
		...
		if (forcedConfigs.containsKey(key))
		{
			return forcedConfigs.get(key);
		}
	...
	{
		return serverconfigs.contains(key);
	}
	...
	public void loadDefaultConfig(FMLPreInitializationEvent event)
	{
		...
		config.getCategory(Constants.CATEGORY_SERVER)
				.setComment("Any key set to true here will ensure that the client is using the configuration set in the 'module' section above.\nThis is useful for enforcing false to 'cheating' keys like silverfish.");
