...
public interface IWailaBlock {
	 ...
	 * world, the player and the location of the block. With that, it is easy to
	 * gather information & tile entities
	 */
	 ...
	 * body to where you mostly want to put informations, and I reserve the tail for
	 * modname display
	 */

	/*
	 * Those 2 methods works exactly the same way, except they are related to a
	 * different zone in Waila HUD. You get in input world, player and the block
	 * location. You also get the itemstack as returned by the default lookup system
	 * or getWailaStack(). ConfigHandler provides the current Waila config state so
	 * you can show/hide elements depending on the configuration. Refer the
	 * ConfigHandler class for more info. currenttip represents the current list of
	 * text lines in the tooltip zone. For example, getWailaHead() will have the
