...
public class UnitDisplayUtils {
	...
	/**
	 * Displays the unit as text. Does handle negative numbers, and will place a negative sign in front of the output string showing this. Use string.replace to remove the negative sign if unwanted
	 */
	public static String getDisplay(double value, ElectricUnit unit, int decimalPlaces, boolean isShort) {
		...
		for (int i = 0; i < MeasurementUnit.values().length; i++) {
			...
			if ((lowerMeasure.below(value) && lowerMeasure.ordinal() == 0) || (lowerMeasure.ordinal() + 1 >= MeasurementUnit.values().length)) {
